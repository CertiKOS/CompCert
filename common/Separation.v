...
Record massert : Type := {
  ...
  (*SACC:*) m_invar_stack: bool;
  m_invar: forall m m', m_pred m -> Mem.unchanged_on m_footprint m m'
                                 -> (*SACC:*) (m_invar_stack = true -> Mem.stack m' = Mem.stack m)
                                 -> m_pred m';
...
Definition massert_imp (P Q: massert) : Prop :=
  (forall m, m_pred P m -> m_pred Q m) /\ 
  (forall b ofs, m_footprint Q b ofs -> m_footprint P b ofs) /\
  (*SACC:*)(*(m_invar_weak Q = true -> m_invar_weak P = true) /\*)
  (*SACC:*)  (m_invar_stack Q = true -> m_invar_stack P = true).
...
(*SACC:*  m_invar_weak := m_invar_weak P || m_invar_weak Q;*)
...
(*SACC:*Global*) Add Morphism sepconj
  with signature massert_imp ==> massert_imp ==> massert_imp
  as sepconj_morph_1.
Proof.
  intros P1 P2 [I [B A]] Q1 Q2 [J [D C]].
...
- intuition auto. rewrite Bool.orb_true_iff in * |- * . tauto.
Qed.
...
- apply (m_invar P) with m; auto. eapply Mem.alloc_unchanged_on; eauto.
  intros. eapply Mem.alloc_stack_unchanged. apply H.
...
(*SACC:*)m_invar_stack := false;
  m_pred := fun m =>
    ...
    /\ Mem.valid_access m chunk b ofs Freeable (*SACC: comment this*)
...
|}.
Next Obligation.
  rename H3 into v. split;[|split]; auto.
...
(*SACC:*)(*m_invar_weak := m_invar_weak P || m_invar_weak Q;*)
(*SACC:*)  m_invar_stack := m_invar_stack P || m_invar_stack Q;
...
(*SACC:*Global*) Add Morphism mconj
  with signature massert_imp ==> massert_imp ==> massert_imp
  as mconj_morph_1.
Proof.
...
(*SACC:*) m_invar_stack := true;
(*SACC:*)(*m_invar_weak := true;*)
...
- apply (m_invar P) with m2; auto.
  ...
  apply V1. omega.
  intros. eapply Mem.store_stack_unchanged. apply STORE.
...
+ apply (m_invar P) with m2; auto. eapply Mem.alloc_unchanged_on; eauto.
  intros. eapply Mem.alloc_stack_unchanged. apply ALLOC2.
+ red; simpl; intros.
...
- apply (m_invar P) with m2; auto. 
  ...
  apply Mem.perm_cur_max. apply Mem.perm_implies with Freeable; auto with mem.
  eapply Mem.free_range_perm; eauto. xomega. 
  intros. eapply Mem.free_stack_unchanged. apply FREE.
...
Lemma alloc_parallel_rule_2:
  ...
  /\ inject_incr j j'
  /\ j' b1 = Some(b2, delta)
(*SACC:*)  /\ inject_separated j j' m1 m2 .
Proof.
  intros. 
...
(*====================================*)
(*====================================*)
(*SACC: Stack-aware CompCert addendum *)
(*====================================*)
(*====================================*)
...
Lemma record_stack_block_parallel_rule:
  forall m1 m1' m2 j P b b' delta sz n,
    j b = Some (b', delta) ->
    ...
    (forall (ofs : Z) (k : perm_kind) (p : permission),
        Mem.perm m2 b' ofs k p -> 0 <= ofs < sz) ->
    (forall bb delta0, j bb = Some (b', delta0) -> bb = b) ->
    forall fa',
      make_singleton_frame_adt b' sz n = fa' ->
    ...
    exists m2',
      ...
      m2' |= minjection j (flat_frameinj (length (Mem.stack m1'))) m1' ** P.
Proof.
  intros m1 m1' m2 j P b b' delta sz n FB INVAR MINJ NIN fa finone (* PUB *) fablocks fasize
         RSB1 PERM0 PERM2 UNIQ fa' fa'eq TTNP SEQ.
  destruct MINJ as (MINJ & PM & DISJ).
  edestruct (Mem.record_push_inject_flat _ _ _ fa fa' m1' MINJ) as (m2' & RSB2 & MINJ'); simpl in *.
  - setoid_rewrite Forall_forall.  intros. destruct x.  simpl in *.
    ...
    simpl.
    intros. exploit (PERM2 (o + delta0)). eapply Mem.perm_inject; eauto.
    intros. 
    assert (sz <= Z.max 0 sz). (apply Z.le_max_r).
    omega.
  ...
  - subst; simpl in *; intros ? ? ? ? ? [B|[]]. inv B. 
    intros. simpl.
    intros. exploit (PERM2 o); eauto.
    intros.
    assert (sz <= Z.max 0 sz). (apply Z.le_max_r).
    omega.
...
Lemma record_stack_block_parallel_rule_2:
  forall m1 m1' m2 j P b b' delta n,
    j b = Some (b', delta) ->
    m_invar_stack P = false ->
    m2 |= minjection j (flat_frameinj (length (Mem.stack m1))) m1 ** P ->
    forall (NIN: ~ in_stack (Mem.stack m2) b') sz sz',
    ...
    stack_equiv (Mem.stack m1) (Mem.stack m2) ->
    exists m2',
      Mem.record_stack_blocks m2 (make_singleton_frame_adt b' sz' n) = Some m2' /\
      m2' |= minjection j (flat_frameinj (length (Mem.stack m1'))) m1' ** P.
Proof.
  intros m1 m1' m2 j P b b' delta n H H0 H1 NIN sz sz' H2 H3 H5 H6 TTNP SEQ.
