...
Record t: Type := mksenv {
  ...
  block_is_volatile_below:
    forall b, block_is_volatile b = Some true -> Block.lt b Block.init;
}.
...
(** ** Linking two global environments *)

Section LINKER_GENV.

Context {F V: Type} {LF: Linker F} {LV: Linker V} `{FI: FundefIsInternal F}.
Context (ge1 ge2: t F V).

Definition link_genv_check (x: ident) (gd1: globdef F V) :=
  match ge2.(genv_defs)!x with
  | None => true
  | Some gd2 =>
      In_dec peq x ge1.(genv_public)
      && In_dec peq x ge2.(genv_public)
      && match link gd1 gd2 with Some _ => true | None => false end
  end.

Definition link_genv :=
  if PTree_Properties.for_all ge1.(genv_defs) link_genv_check
  then
    Some
    {|
      genv_public := ge1.(genv_public) ++ ge2.(genv_public);
      genv_defs := PTree.combine link_prog_merge ge1.(genv_defs) ge2.(genv_defs);
    |}
  else
    None.

Record linkorder_genv :=
  {
    linkorder_genv_public:
      incl ge1.(genv_public) ge2.(genv_public);
    linkorder_genv_defs:
      forall id gd1,
        ge1.(genv_defs)!id = Some gd1 ->
        exists gd2,
          ge2.(genv_defs)!id = Some gd2 /\
          linkorder gd1 gd2 /\
          (~ In id ge2.(genv_public) -> gd2 = gd1);
  }.

End LINKER_GENV.

Program Instance Linker_genv F V {LF: Linker F} {LV: Linker V} {FI: FundefIsInternal F}: Linker (t F V) :=
  {
    link := link_genv;
    linkorder := linkorder_genv;
  }.
Next Obligation. (* linkorder reflexive *)
  split.
  - apply incl_refl.
  - intros id gd Hgd.
    exists gd; intuition.
...
Qed.
Next Obligation. (* linkorder transitive *)
  destruct H as [Hp12 Hd12], H0 as [Hp23 Hd23].
  split.
  - eapply incl_tran; eauto.
  - intros id gd1 Hgd1.
    specialize (Hd12 id).
    specialize (Hd23 id).
    edestruct Hd12 as (gd2 & Hgd2 & Hgd12 & Hgdp12); eauto.
    edestruct Hd23 as (gd3 & Hgd3 & Hgd23 & Hgdp23); eauto.
    exists gd3; intuition idtac.
    + eapply linkorder_trans; eauto.
    + destruct (In_dec ident_eq id (genv_public y)) as [Hid|Hid].
      * apply Hp23 in Hid.
        elim H; eauto.
      * specialize (Hgdp12 Hid).
        congruence.
Qed.
Next Obligation. (* link vs. linkorder *)
  unfold link_genv in H.
  destruct PTree_Properties.for_all eqn:Hcheck; inv H.
  split.
  - split; simpl.
    ...
    + intros id gd1 Hgd1.
      ...
      specialize (Hcheck id gd1 Hgd1).
      unfold link_genv_check in *.
      destruct (y.(genv_defs)!id) as [gd2|] eqn:Hgd2.
      * rewrite !andb_true_iff in Hcheck.
        destruct Hcheck as [[Hp1 Hp2] Hl].
        destruct link as [gd|] eqn:Hgd; try discriminate.
        destruct in_dec; try discriminate.
        destruct in_dec; try discriminate.
        apply link_linkorder in Hgd. destruct Hgd as [H1 H2].
        exists gd; intuition idtac.
        elim H. apply in_app; eauto.
      * exists gd1; intuition idtac.
        apply linkorder_refl.
  - split; simpl.
    ...
    + intros id gd2 Hgd2.
      ...
      rewrite Hgd2. simpl.
      rewrite PTree_Properties.for_all_correct in Hcheck.
      destruct (x.(genv_defs)!id) as [gd1|] eqn:Hgd1; simpl.
      * specialize (Hcheck id gd1 Hgd1).
        unfold link_genv_check in *.
        rewrite Hgd2 in Hcheck.
        rewrite !andb_true_iff in Hcheck.
        destruct Hcheck as [[Hp1 Hp2] Hl].
        destruct link as [gd|] eqn:Hgd; try discriminate.
        destruct in_dec; try discriminate.
        ...
        exists gd; intuition idtac.
        elim H. apply in_app; eauto.
      * exists gd2; intuition idtac.
        apply linkorder_refl.
Qed.

(** ** Commutation theorem *)

(** If both the linking of programs and their derived global
  environments are defined, then the global environment of the linked
  program is equivalent to the linked global environment. *)

Section GENV_GLOBALENV_LINK.
...
Proof.
  intros i.
  apply link_prog_inv in Hp.
  destruct Hp as (_ & _ & Hp').
  rewrite Hp'.
  rewrite prog_defmap_elements.
  reflexivity.
Qed.

Record genv_islinked (ge1 ge2 ge: Genv.t F V) (ii: list ident) :=
  {
    is_linked_at i:
      In i ii ->
      ge.(genv_defs)!i = link_prog_merge ge1.(genv_defs)!i ge2.(genv_defs)!i;
    not_linked_at i:
      ~ In i ii ->
      ge.(genv_defs)!i = None;
  }.

Definition def_islinked (ge1 ge2: Genv.t F V) '(i, def) :=
  link_prog_merge ge1.(genv_defs)!i ge2.(genv_defs)!i = Some def.

Lemma add_global_link (ge1 ge2 ge ge': Genv.t F V) ii i (d: globdef F V):
  genv_islinked ge1 ge2 ge ii ->
  def_islinked ge1 ge2 (i, d) ->
  add_global ge (i, d) = ge' ->
  genv_islinked ge1 ge2 ge' (ii ++ i :: nil).
Proof.
Admitted.

Lemma linked_prog_defs_linked:
  Forall (def_islinked (globalenv p1) (globalenv p2)) (prog_defs p).
Proof.
Admitted.

Lemma globalenv_islinked :
  genv_islinked
    (globalenv p1)
    (globalenv p2)
    (globalenv p)
    (map fst (prog_defs p)).
Proof.
  unfold globalenv at 3.
  generalize linked_prog_defs_linked.
  rewrite Forall_forall.
  generalize (prog_defs p). intro l. pattern l.
  apply rev_ind; clear l.
  - intros _.
    split.
    + contradiction.
    + intros i _. simpl.
      apply PTree.gempty.
  - intros [i def] l IHl H.
    setoid_rewrite in_app in H. simpl in H.
    rewrite map_app. simpl.
    eapply add_global_link; eauto.
    unfold add_globals. rewrite fold_left_app. simpl. reflexivity.
Qed.

Lemma genv_prog_link ge :
  link (globalenv p1) (globalenv p2) = Some ge ->
  match_genvs eq (globalenv p) ge.
Proof.
  simpl. unfold link_genv.
  destruct PTree_Properties.for_all; inversion 1; clear H; subst.
  rewrite !globalenv_defs.
  constructor. intro i. simpl.
  rewrite globalenv_defs, prog_defmap_link.
  destruct (_ ! i); constructor; auto.
Qed.

End GENV_GLOBALENV_LINK.
