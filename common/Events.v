...
(* stack *)
- inv H. inv H0. auto.
  symmetry; eapply Mem.store_stack_unchanged; eauto.
(* perm *)
- inv H.  inv H1. tauto.
...
(* stack *)
- inv H.
  rewrite  (Mem.store_stack_unchanged _ _ _ _ _ _ H1).
  symmetry; eapply Mem.alloc_stack_unchanged; eauto.
(* perm *)
- inv H.  
  assert (b <> b0).
  intro. subst.
  eapply Mem.in_stack_valid in H0. eapply Mem.fresh_block_alloc in H0; eauto.
  split; intros.
...
- inv H; inv H0.
  ...
  split. constructor. intuition congruence.
(* stack *)
- inv H. exploit Mem.free_stack_unchanged. eauto. 
  intro. rewrite H. reflexivity.
(* perm *)
- inv H.  
  split; intros.
  eapply Mem.perm_free_3; eauto.
  eapply Mem.in_stack_valid in H0.
...
- (* stack *)
  intros; inv H.
  symmetry; eapply Mem.storebytes_stack_unchanged; eauto.
- (* perm *)
  intros. inv H.
...
(*SACC:
Parameter builtin_functions_sem: String.string -> signature -> extcall_sem;
Parameter runtime_functions_sem: String.string -> signature -> extcall_sem;
*)
...
(*SACC:
Axiom builtin_functions_properties:
    forall id sg, extcall_properties (builtin_functions_sem id sg) sg.

Axiom runtime_functions_properties:
...
(*| EF_free              => extcall_free_sem *) (*SACC: comment free*)
...
(* apply extcall_free_ok. *) (*SACC: comment free *)
...
Ltac rewrite_stack_blocks :=
  ...
  | H:Mem.store _ _ _ _ _ = Some ?m |- context [ Mem.stack ?m ] => rewrite (Mem.store_stack_unchanged _ _ _ _ _ _ H)
  | H:Mem.storebytes _ _ _ _ = Some ?m |- context [ Mem.stack ?m ] => rewrite (Mem.storebytes_stack_unchanged _ _ _ _ _ H)
  | H:Mem.storev _ _ _ _ = Some ?m |- context [ Mem.stack ?m ] => rewrite (Mem.storev_stack_unchanged _ _ _ _ _ H)
  | H:external_call _ _ _ _ _ _ ?m |- context [ Mem.stack ?m ] => rewrite <- (external_call_stack_blocks _ _ _ _ _ _ _ H)
  | H:Mem.free_list _ _ = Some ?m |- context [ Mem.stack ?m ] => rewrite (Mem.freelist_stack_unchanged _ _ _ H)
  | H:Mem.free _ _ _ _ = Some ?m |- context [ Mem.stack ?m ] => rewrite (Mem.free_stack_unchanged _ _ _ _ _ H)
  ...
  | H: Genv.init_mem _ _ _ = Some ?m |- context [Mem.stack ?m] => rewrite (Genv.init_mem_stack _ _ _ _ H)
  ...
  | H:Mem.unrecord_stack_block ?m1 = Some ?m
    ...
    let f := fresh "f" in
    let EQ := fresh "EQ" in
    destruct (Mem.unrecord_stack_eq _ _ H) as (f, EQ); replace (Mem.stack m) with (tl (Mem.stack m1))
...
Ltac rewnb :=
  repeat
    ...
    | H: Genv.init_mem _ _ _ = Some ?m |- context [Mem.nextblock ?m] =>
      rewrite <- (Genv.init_mem_genv_next _ _ _ _ H)
