...
Require Import Csharpminor_old Cminor_old.
...
Definition transl_constant (cst: Csharpminor_old.constant): constant :=
  match cst with
  | Csharpminor_old.Ointconst n =>
      Ointconst n
  | Csharpminor_old.Ofloatconst n =>
      Ofloatconst n
  | Csharpminor_old.Osingleconst n =>
      Osingleconst n
  | Csharpminor_old.Olongconst n =>
      Olongconst n
  end.

(** Translation of expressions.  *)

Fixpoint transl_expr (cenv: compilenv) (e: Csharpminor_old.expr)
                     {struct e}: res expr :=
  match e with
  | Csharpminor_old.Evar id =>
      OK (Evar id)
  | Csharpminor_old.Eaddrof id =>
      OK (var_addr cenv id)
  | Csharpminor_old.Econst cst =>
      OK (Econst (transl_constant cst))
  | Csharpminor_old.Eunop op e1 =>
      do te1  <- transl_expr cenv e1;
      OK (Eunop op te1)
  | Csharpminor_old.Ebinop op e1 e2 =>
      do te1 <- transl_expr cenv e1;
      do te2 <- transl_expr cenv e2;
      OK (Ebinop op te1 te2)
  | Csharpminor_old.Eload chunk e =>
      do te <- transl_expr cenv e;
      OK (Eload chunk te)
  end.

Fixpoint transl_exprlist (cenv: compilenv) (el: list Csharpminor_old.expr)
...
Fixpoint transl_stmt (cenv: compilenv) (xenv: exit_env) (s: Csharpminor_old.stmt)
                     {struct s}: res stmt :=
  match s with
  | Csharpminor_old.Sskip =>
      OK Sskip
  | Csharpminor_old.Sset id e =>
      do te <- transl_expr cenv e;
      OK (Sassign id te)
  | Csharpminor_old.Sstore chunk e1 e2 =>
      do te1 <- transl_expr cenv e1;
      do te2 <- transl_expr cenv e2;
      OK (Sstore chunk te1 te2)
  | Csharpminor_old.Scall optid sig e el =>
      do te <- transl_expr cenv e;
      do tel <- transl_exprlist cenv el;
      OK (Scall optid sig te tel)
  | Csharpminor_old.Sbuiltin optid ef el =>
      do tel <- transl_exprlist cenv el;
      OK (Sbuiltin optid ef tel)
  | Csharpminor_old.Sseq s1 s2 =>
      do ts1 <- transl_stmt cenv xenv s1;
      do ts2 <- transl_stmt cenv xenv s2;
      OK (Sseq ts1 ts2)
  | Csharpminor_old.Sifthenelse e s1 s2 =>
      do te <- transl_expr cenv e;
      do ts1 <- transl_stmt cenv xenv s1;
      do ts2 <- transl_stmt cenv xenv s2;
      OK (Sifthenelse te ts1 ts2)
  | Csharpminor_old.Sloop s =>
      do ts <- transl_stmt cenv xenv s;
      OK (Sloop ts)
  | Csharpminor_old.Sblock s =>
      do ts <- transl_stmt cenv (true :: xenv) s;
      OK (Sblock ts)
  | Csharpminor_old.Sexit n =>
      OK (Sexit (shift_exit xenv n))
  | Csharpminor_old.Sswitch long e ls =>
      let (tbl, dfl) := switch_table ls O in
      do te <- transl_expr cenv e;
      transl_lblstmt cenv (switch_env ls xenv) ls (Sswitch long te tbl dfl)
  | Csharpminor_old.Sreturn None =>
      OK (Sreturn None)
  | Csharpminor_old.Sreturn (Some e) =>
      do te <- transl_expr cenv e;
      OK (Sreturn (Some te))
  | Csharpminor_old.Slabel lbl s =>
      do ts <- transl_stmt cenv xenv s; OK (Slabel lbl ts)
  | Csharpminor_old.Sgoto lbl =>
      OK (Sgoto lbl)
  end

with transl_lblstmt (cenv: compilenv) (xenv: exit_env) (ls: Csharpminor_old.lbl_stmt) (body: stmt)
                    {struct ls}: res stmt :=
  match ls with
  | Csharpminor_old.LSnil =>
      OK (Sseq (Sblock body) Sskip)
  | Csharpminor_old.LScons _ s ls' =>
      do ts <- transl_stmt cenv xenv s;
...
Definition build_compilenv (f: Csharpminor_old.function) : compilenv * Z :=
  assign_variables (PTree.empty Z, 0) (VarSort.sort (Csharpminor_old.fn_vars f)).
...
Definition transl_funbody
      (cenv: compilenv) (stacksize: Z) (f: Csharpminor_old.function): res function :=
   do tbody <- transl_stmt cenv nil f.(Csharpminor_old.fn_body);
       OK (mkfunction
              (Csharpminor_old.fn_sig f)
              (Csharpminor_old.fn_params f)
              (Csharpminor_old.fn_temps f)
              stacksize
              tbody).

Definition transl_function (f: Csharpminor_old.function): res function :=
...
Definition transl_fundef (f: Csharpminor_old.fundef): res fundef :=
  transf_partial_fundef transl_function f.

Definition transl_program (p: Csharpminor_old.program) : res program :=
