...
Definition make_memcpy (ce: composite_env) (dst src: expr) (ty: type) :=
  do sz <- sizeof ce ty;
  OK (Sbuiltin None (EF_memcpy sz (Ctypes_old.alignof_blockcopy ce ty))
                    (dst :: src :: nil)).
...
Definition transl_unop (op: Cop_old.unary_operation) (a: expr) (ta: type) : res expr :=
  match op with
  | Cop_old.Onotbool => make_notbool a ta
  | Cop_old.Onotint => make_notint a ta
  | Cop_old.Oneg => make_neg a ta
  | Cop_old.Oabsfloat => make_absfloat a ta
  end.

Definition transl_binop (ce: composite_env)
                        (op: Cop_old.binary_operation)
                        (a: expr) (ta: type)
                        (b: expr) (tb: type) : res expr :=
  match op with
  | Cop_old.Oadd => make_add ce a ta b tb
  | Cop_old.Osub => make_sub ce a ta b tb
  | Cop_old.Omul => make_mul a ta b tb
  | Cop_old.Odiv => make_div a ta b tb
  | Cop_old.Omod => make_mod a ta b tb
  | Cop_old.Oand => make_and a ta b tb
  | Cop_old.Oor  => make_or a ta b tb
  | Cop_old.Oxor => make_xor a ta b tb
  | Cop_old.Oshl => make_shl a ta b tb
  | Cop_old.Oshr => make_shr a ta b tb
  | Cop_old.Oeq => make_cmp Ceq a ta b tb
  | Cop_old.One => make_cmp Cne a ta b tb
  | Cop_old.Olt => make_cmp Clt a ta b tb
  | Cop_old.Ogt => make_cmp Cgt a ta b tb
  | Cop_old.Ole => make_cmp Cle a ta b tb
  | Cop_old.Oge => make_cmp Cge a ta b tb
  end.
...
Fixpoint transl_expr (ce: composite_env) (a: Clight_old.expr) {struct a} : res expr :=
  match a with
  | Clight_old.Econst_int n _ =>
      OK(make_intconst n)
  | Clight_old.Econst_float n _ =>
      OK(make_floatconst n)
  | Clight_old.Econst_single n _ =>
      OK(make_singleconst n)
  | Clight_old.Econst_long n _ =>
      OK(make_longconst n)
  | Clight_old.Evar id ty =>
      make_load (Eaddrof id) ty
  | Clight_old.Etempvar id ty =>
      OK(Evar id)
  | Clight_old.Ederef b ty =>
      do tb <- transl_expr ce b;
      make_load tb ty
  | Clight_old.Eaddrof b _ =>
      transl_lvalue ce b
  | Clight_old.Eunop op b _ =>
      do tb <- transl_expr ce b;
      transl_unop op tb (typeof b)
  | Clight_old.Ebinop op b c _ =>
      do tb <- transl_expr ce b;
      do tc <- transl_expr ce c;
      transl_binop ce op tb (typeof b) tc (typeof c)
  | Clight_old.Ecast b ty =>
      do tb <- transl_expr ce b;
      make_cast (typeof b) ty tb
  | Clight_old.Efield b i ty =>
      do tb <- transl_expr ce b;
      do addr <- make_field_access ce (typeof b) i tb;
      make_load addr ty
  | Clight_old.Esizeof ty' ty =>
      do sz <- sizeof ce ty'; OK(make_ptrofsconst sz)
  | Clight_old.Ealignof ty' ty =>
...
with transl_lvalue (ce: composite_env) (a: Clight_old.expr) {struct a} : res expr :=
  match a with
  | Clight_old.Evar id _ =>
      OK (Eaddrof id)
  | Clight_old.Ederef b _ =>
      transl_expr ce b
  | Clight_old.Efield b i ty =>
      do tb <- transl_expr ce b;
...
Fixpoint transl_arglist (ce: composite_env) (al: list Clight_old.expr) (tyl: typelist)
...
Fixpoint typlist_of_arglist (al: list Clight_old.expr) (tyl: typelist)
                            {struct al}: list AST_old.typ :=
  match al, tyl with
  | nil, _ => nil
  | a1 :: a2, Tcons ty1 ty2 =>
...
Fixpoint transl_statement (ce: composite_env) (tyret: type) (nbrk ncnt: nat)
                          (s: Clight_old.statement) {struct s} : res stmt :=
  match s with
  | Clight_old.Sskip =>
      OK Sskip
  | Clight_old.Sassign b c =>
  ...
  | Clight_old.Sbreak =>
      OK (Sexit nbrk)
  | Clight_old.Scontinue =>
      OK (Sexit ncnt)
  | Clight_old.Sreturn (Some e) =>
      do te <- transl_expr ce e;
      do te' <- make_cast (typeof e) tyret te;
      OK (Sreturn (Some te'))
  | Clight_old.Sreturn None =>
      OK (Sreturn None)
  | Clight_old.Sswitch a sl =>
...
Definition signature_of_function (f: Clight_old.function) :=
  {| sig_args := map typ_of_type (map snd (Clight_old.fn_params f));
     sig_res  := opttyp_of_type (Clight_old.fn_return f);
     sig_cc   := Clight_old.fn_callconv f |}.

Definition transl_function (ce: composite_env) (f: Clight_old.function) : res function :=
  do tbody <- transl_statement ce f.(Clight_old.fn_return) 1%nat 0%nat (Clight_old.fn_body f);
  do tvars <- mmap (transl_var ce) (Clight_old.fn_vars f);
  OK (mkfunction
       (signature_of_function f)
       (map fst (Clight_old.fn_params f))
       tvars
       (map fst (Clight_old.fn_temps f))
       tbody).

Definition transl_fundef (ce: composite_env) (id: ident) (f: Clight_old.fundef) : res fundef :=
  match f with
  | Internal g =>
      do tg <- transl_function ce g; OK(AST_old.Internal tg)
  | External ef args res cconv =>
      if signature_eq (ef_sig ef) (signature_of_type args res cconv)
      then OK(AST_old.External ef)
...
Definition transl_program (p: Clight_old.program) : res program :=
