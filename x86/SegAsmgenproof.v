...
Require Import SegAsm SegAsmBuiltin SegAsmgen SegAsmProgram SegMemAccessors.
Require Import Segment.
Require Import Events.
Require Import StackADT.
Require Import Linking Errors.
Require Import Globalenvs SegAsmGlobenv.
...
(** Lemmas about SegAsmgen that are useful for proving invariants *)
...
(*   2. The mapping from segment ids to segment blocks provided by the SegAsm environment *)
...
Lemma transl_instrs_gen_valid_code_labels : forall instrs (gmap: GID_MAP_TYPE) i (tp:SegAsm.program) sid ofs1 ofs2 ofs' instrs',
...
Lemma transl_fun_gen_valid_code_labels : forall gmap i f f' (tp:SegAsm.program),
  (forall id b, gmap id = Some b -> In (fst b) (map segid (list_of_segments tp))) ->
...
Lemma transl_globdefs_gen_valid_code_labels : forall defs gmap tdefs (tp:SegAsm.program),
  (forall id b, gmap id = Some b -> In (fst b) (map segid (list_of_segments tp))) ->
...
Definition match_prog (p: Asm.program) (tp: SegAsm.program) :=
...
Definition find_symbol_block_bound (ge:SegAsm.genv) :=
...
Lemma external_in_defs_inversion : forall defs f sb i (ge:SegAsm.genv)
...
Lemma external_find_symbol_det : forall defs (ge:SegAsm.genv) i b1 b2 o1 o2 f sb
  (IN: In (i, Some (Gfun (External f)), sb) defs)
  (NPT: list_norepet (map (fun '(i, _, _) => i) defs))
...
Lemma add_globals_find_symbol_external: forall defs i f sb b ofs (ge:SegAsm.genv)
     (NPT: list_norepet (map (fun '(i, _, _) => i) defs))
     (IN: In (i, Some (Gfun (External f)), sb) defs)
...
Lemma transf_initial_states : forall rs (SELF: forall j, forall r : PregEq.t, Val.inject j (rs r) (rs r)) st1,
    ...
    exists st2, SegAsm.initial_state tprog rs st2 /\ match_states st1 st2.
Proof.
...
Existing Instance SegMemAccessors.mem_accessors_default.
...
Lemma eval_builtin_arg_inject : forall j m m' rs rs' sp sp' arg varg
    ...
    exists varg', SegAsmBuiltin.eval_builtin_arg _ _ _ _ tge rs' sp' m' arg varg' /\
...
Lemma eval_builtin_args_inject : forall j m m' rs rs' sp sp' args vargs
    ...
    exists vargs', SegAsmBuiltin.eval_builtin_args _ _ _ _ tge rs' sp' m' args vargs' /\
...
Proof.
  ...
  - exploit eval_builtin_arg_inject; eauto.
    ...
    unfold SegAsmBuiltin.eval_builtin_args.
...
Lemma eval_addrmode32_inject: forall j a rs1 rs2,
    match_inj j ->
    regset_inject j rs1 rs2 ->
    Val.inject j (Asm.eval_addrmode32 ge a rs1) (SegMemAccessors.eval_addrmode32 tge a rs2).
Proof.
  intros. unfold Asm.eval_addrmode32, SegMemAccessors.eval_addrmode32.
  destruct a. 
...
Lemma eval_addrmode64_inject: forall j a rs1 rs2,
    match_inj j ->
    regset_inject j rs1 rs2 ->
    Val.inject j (Asm.eval_addrmode64 ge a rs1) (SegMemAccessors.eval_addrmode64 tge a rs2).
Proof.
  intros. unfold Asm.eval_addrmode32, SegMemAccessors.eval_addrmode32.
  destruct a. 
  destruct base, ofs, const; simpl in *.
...
Lemma eval_addrmode_inject: forall j a rs1 rs2,
    match_inj j ->
    regset_inject j rs1 rs2 ->
    Val.inject j (Asm.eval_addrmode ge a rs1) (SegMemAccessors.eval_addrmode tge a rs2).
Proof.
...
Lemma exec_load_step: forall j rs1 rs2 m1 m2 rs1' m1' sz chunk rd a
    ...
    exists rs2' m2',
      SegMemAccessors.exec_load tge chunk m2 a rs2 rd sz = Next rs2' m2' /\
...
Lemma exec_store_step: forall j rs1 rs2 m1 m2 rs1' m1' sz chunk r a dregs
    ...
    exists rs2' m2',
      SegMemAccessors.exec_store tge chunk m2 a rs2 r dregs sz = Next rs2' m2' /\
      match_states (State rs1' m1') (State rs2' m2').
Proof.
...
Ltac solve_match_states :=
  ...
  | [ |- (SegAsm.exec_instr _ _ _ _ = Next _ _) /\ match_states _ _ ] =>
...
Lemma goto_tbl_label_inject : forall id tbl l b f j rs1 rs2 m1 m2 rs1' m1' i ofs
    ...
    exists rs2',
      SegAsm.goto_label tge id l ((rs2 # RAX <- Vundef) # RDX <- Vundef) m2 = Next rs2' m2 /\
      regset_inject j rs1' rs2' /\ Mem.inject j (def_frame_inj m1') m1' m2.
Proof.
...
Lemma add_globals_pres_senv :
  forall (defs : list (ident * option gdef * segblock)) (ge : SegAsm.genv),
...
Axiom exec_instr_step : forall j rs1 rs2 m1 m2 rs1' m1' i i' id sid ofs ofs' f b
                        (MINJ: Mem.inject j (def_frame_inj m1) m1 m2)
                        (MATCHSMINJ: match_inj j)
                        (* (GINJFLATMEM: globs_inj_into_flatmem j) *)
                        (INSTRINTERNAL: valid_instr_offset_is_internal j)
    ...
    exists rs2' m2',
      SegAsm.exec_instr tge i' rs2 m2 = Next rs2' m2' /\
      match_states (State rs1' m1') (State rs2' m2').
(* Proof. *)
(*   intros. *)
...
Proof.
  ...
  - (* Internal step *)
    ...
    eapply SegAsm.exec_step_internal; eauto.
  ...
  - (* Builtin *)
    ...
    exploit (fun b ofs => SegAsm.exec_step_builtin tge id b ofs
  ...
  - (* External call *)
    ...
    exploit (fun ofs => SegAsm.exec_step_external tge b2 ofs ef args2 res'); eauto.
...
Lemma transf_final_states:
  forall st1 st2 r,
  match_states st1 st2 -> Asm.final_state st1 r -> SegAsm.final_state st2 r.
Proof.
...
Theorem transf_program_correct:
  forward_simulation (RealAsm.semantics prog (Pregmap.init Vundef)) (SegAsm.semantics tprog (Pregmap.init Vundef)).
Proof.
...
Lemma transl_instrs_id_code:
  forall c i  ii sbb id s i0 x x0,
    SegAsmgen.transl_instrs i s i0 c = OK (x, x0) ->
    In (ii, sbb, id) x0 ->
...
Proof.
  ...
  destruct H0.
  - subst.
    unfold SegAsmgen.transl_instr in EQ.
...
Lemma in_transl_globdefs_code:
  forall defs g tdefs i tdef sb,
    SegAsmgen.transl_globdefs g defs = OK tdefs ->
    In (i, Some tdef, sb) tdefs ->
    exists def,
      In (i, def) defs /\ SegAsmgen.transl_globdef g i def = OK (i, Some tdef, sb).
Proof.
  induction defs; simpl; intros; eauto.
  - inv H; easy.
  - repeat destr_in H. monadInv H2.
    ...
    assert (i = i0).
    {
      unfold SegAsmgen.transl_globdef in EQ. repeat destr_in EQ.
      monadInv H0. auto.
    } subst.
    eexists; split; eauto.
...
Lemma transl_fun_id_code:
  forall g i f1 f2 ii sbb id,
    SegAsmgen.transl_fun g i f1 = OK f2 ->
    In (ii, sbb, id) (fn_code f2) ->
    exists s i0,
      g i = Some (s, i0) /\
      segblock_id sbb = s /\ i = id.
Proof.
  unfold SegAsmgen.transl_fun.
...
Lemma transl_instrs_back:
  forall c i  ii sbb id s i0 x x0,
    ...
    In (ii, sbb, id) x0 ->
    exists ofs ii1,
      SegAsmgen.transl_instr ofs i s ii1 = OK (ii, sbb,id) /\
      In ii1 c.
Proof.
  induction c; simpl; intros; eauto.
  inv H. easy.
...
Proof.
  ...
  unfold SegAsmgen.transf_program.
  repeat destr.
  ...
  - intros ((ii & sbb) & id) INc.
    ...
    unfold SegAsmgen.transl_globdef in TG. repeat destr_in TG.
    monadInv H0.
    ...
    red in wf_prog_valid_labels.
    rewrite Forall_forall in wf_prog_valid_labels.
    unfold SegAsmgen.transl_fun in EQ0.
