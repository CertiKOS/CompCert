...
Definition link_rodata_reloctable (p1 p2 p: program) : option reloctable :=
  ...
  match SecTable.get sec_rodata_id (prog_sectable p1)
  with
  | Some rodata_sec1=>
    let t1 := reloctable_rodata (prog_reloctables p1) in
    let t2 := reloctable_rodata (prog_reloctables p2) in
    let rdsz := sec_size rodata_sec1 in
