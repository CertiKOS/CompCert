...
Proof.
  ...
  destruct (Forall_dec _ (valid_relocentry_dec) (get_reloctable RELOC_BSS rtbls)).
    ...
    if strtable_empty_dec (prog_strtable p) then
      if norepet_symbs_dec (prog_symbtable p) then
        if reloctables_ok_dec (prog_reloctables p) then
          ...
          let bssrelocsec := create_reloctable_section (reloctable_bss (prog_reloctables p)) in
          ...
          let p' :=
              {| prog_defs := p.(prog_defs);
                 ...
                 prog_sectable :=
                   p.(prog_sectable) ++ [strsec; symsec; bssrelocsec; rodatarelocsec; datarelocsec; coderelocsec; shstrsec];
          ...
          if beq_nat len 11 then
            OK p'
          else
            Error [MSG "In TablesEncode: number of sections is incorrect (not 11): "; POS (Pos.of_nat len)]
