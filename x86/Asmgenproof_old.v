...
Require Import Op_old Locations_old Mach_old Conventions_old Asm_old.
Require Import Asmgen_old Asmgenproof0_old Asmgenproof1_old.
Require AsmFacts_old.

Definition match_prog (p: Mach_old.program) (tp: Asm_old.program) :=
...
Variable prog: Mach_old.program.
Variable tprog: Asm_old.program.
...
<<
        ...
        Mach code ------------------------ Asm instr sequence
            |                                          |
            | Mach_old.find_label lbl       find_label lbl |
            |                                          |
            v                                          v
...
Proof.
  unfold mk_mov; unfold Asmgen_old.mk_mov; intros.
...
Remark mk_intconv_label:
  ...
  tail_nolabel k c.
Proof.
  unfold mk_intconv; unfold Asmgen_old.mk_intconv; intros.
...
Proof.
Opaque loadind.
  unfold transl_instr; unfold Asmgen_old.transl_instr; intros; destruct i; TailNoLabel.
...
Lemma transl_instr_label':
  ...
  transl_instr f i ep k = OK c ->
  find_label lbl c = if Mach_old.is_label lbl i then Some k else find_label lbl k.
Proof.
...
Lemma transl_code_label:
  ...
  transl_code f c ep = OK tc ->
  match Mach_old.find_label lbl c with
...
Proof.
  ...
  generalize (Mach_old.is_label_correct lbl a).
  destruct (Mach_old.is_label lbl a); intros.
...
Lemma transl_find_label:
  forall lbl f tf,
  transf_function f = OK tf ->
  match Mach_old.find_label lbl f.(Mach_old.fn_code) with
...
Lemma find_label_goto_label:
  ...
  transf_function f = OK tf ->
  rs PC = Vptr b ofs ->
  Mach_old.find_label lbl f.(Mach_old.fn_code) = Some c' ->
  exists tc', exists rs',
...
Proof.
  ...
  intros. eapply Asmgenproof0_old.return_address_exists; eauto.
- intros. exploit transl_instr_label; eauto.
...
*)
 ...
 Inductive match_states: Mach_old.state -> Asm_old.state -> Prop :=
  | match_states_intro:
      ...
      match_states (Mach_old.State s fb (Vptr sp Ptrofs.zero) c ms m)
                   (Asm_old.State rs m')
  | match_states_call:
      forall s fb ms m m' rs
        ...
        (SAMEPERM: forall b o k p, in_stack (Mem.stack m) b -> (Mem.perm m b o k p <-> Mem.perm m' b o k p))
        (SAMEADT: (Mem.stack m) = (Mem.stack m')),
      match_states (Mach_old.Callstate s fb ms m)
                   (Asm_old.State rs m')
  | match_states_return:
      ...
      match_states (Mach_old.Returnstate s ms m)
                   (Asm_old.State rs m').
...
Lemma exec_straight_steps:
  ...
  match_states (Mach_old.State s fb (Vptr sp Ptrofs.zero) c ms2 m2) st'.
Proof.
...
(** We need to show that, in the simulation diagram, we cannot
  ...
  transition from [Mach_old.Returnstate] to [Mach_old.State].
  So, the following integer measure will suffice to rule out
  the unwanted behaviour. *)

Definition measure (s: Mach_old.state) : nat :=
  match s with
  | Mach_old.State _ _ _ _ _ _ => 0%nat
  | Mach_old.Callstate _ _ _ _ => 0%nat
  | Mach_old.Returnstate _ _ _ => 1%nat
  end.
...
Lemma agree_undef_regs_parallel:
  forall l sp rs rs0,
    agree rs sp rs0 ->
    agree (Mach_old.undef_regs l rs) sp
...
Qed.
  ...
  Lemma check_alloc_frame_correct:
    forall fi f,
      Mach_old.check_alloc_frame fi f ->
      exists x, check_alloc_frame fi = left x.
  Proof.
    unfold Mach_old.check_alloc_frame.
    intros fi f A.
  ...
  Lemma load_parent_pointer_correct:
    forall (tge: genv) fn (rd: ireg) x m1 (rs1: regset) s fb sp c f
      ...
      (RD: rd <> RSP)
      ms1
      (LP: call_stack_consistency ge init_sg init_stk (Mach_old.State s fb (Vptr sp Ptrofs.zero) c ms1 m1))
      m2 (EQSTK: Mem.stack m1 = Mem.stack m2),
      exists rs2 sp,
        exec_straight init_stk tge fn ((Pload_parent_pointer rd (Mach_old.fn_stacksize f)) :: x) rs1 m2 x rs2 m2 /\
  ...
  Proof.
    ...
    exists (nextinstr (rs1#rd <- (parent_sp (Mem.stack m1))) 
                 (Ptrofs.repr (instr_size ((Pload_parent_pointer rd (Mach_old.fn_stacksize f)))))), (Vptr bisp Ptrofs.zero).
    split; [|split].
    - constructor.
...
Proof.
  ...
  generalize (code_tail_code_size (Pallocframe (Mach_old.fn_stacksize trf) (fn_frame_pubrange trf) (fn_retaddr_ofs trf) :: y ++ icall :: nil) x0).
...
Theorem step_simulation:
  forall S1 t S2, Mach_old.step init_ra return_address_offset invalidate_frame2 ge S1 t S2 ->
...
- (* Mtailcall *)
  ...
  assert (CTF: check_top_frame m'0 (Some stk) (Mach_old.fn_stacksize f) = true).
  {
...
+ (* Indirect call *)
  ...
  set (sz := (Ptrofs.repr
                   (instr_size
                      ((Pfreeframe ((Mach_old.fn_stacksize tf0)) (fn_retaddr_ofs tf0)))))) in *.
...
- (* Mjumptable *)
  ...
  apply plus_one. econstructor; eauto.
  eapply find_instr_tail; eauto.
  unfold exec_instr; simpl. rewrite <- H9.  unfold Mach_old.label in H0; unfold label; rewrite H0. eexact A.
  econstructor; eauto.
...
- (* Mreturn *)
  ...
  + eapply plus_left. eapply exec_step_internal. eauto.
    ...
    apply star_one. eapply exec_step_internal.
    transitivity (Val.offset_ptr rs0#PC (instr_size_in_ptrofs (Pfreeframe (Mach_old.fn_stacksize tf0) (fn_retaddr_ofs tf0)))). 
...
Lemma transf_initial_states:
  forall st1, Mach_old.initial_state prog st1 ->
         exists st2, Asm_old.initial_state tprog st2 /\ match_states Vnullptr st1 st2
    ...
    + split. econstructor; eauto.
      ...
      * split; constructor.
        ...
        -- repeat rewrite_stack_blocks. simpl. constructor.
           ...
           replace (Stacklayout_old.fe_ofs_arg) with 0. intros; omega. reflexivity.
           reflexivity.
...
Lemma transf_final_states:
  forall st1 st2 r,
  match_states Vnullptr st1 st2 -> Mach_old.final_state st1 r -> Asm_old.final_state st2 r.
Proof.
...
Hypothesis frame_correct:
  forall (fb : block) (f : Mach_old.function),
    Genv.find_funct_ptr ge fb = Some (Internal f) ->
    0 < Mach_old.fn_stacksize f.

Theorem transf_program_correct:
  forward_simulation (Mach_old.semantics2 return_address_offset prog)
                     (Asm_old.semantics tprog
                                    ((Some (make_singleton_frame_adt (Genv.genv_next ge) 0 0), nil) :: nil)).
Proof.
