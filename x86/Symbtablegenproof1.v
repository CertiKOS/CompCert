...
Lemma get_symbentry_pres_internal_prop : forall rdid did cid id rdsz dsz csz def,
    is_def_internal is_fundef_internal def = 
    is_symbentry_internal (get_symbentry rdid did cid rdsz dsz csz id def).
Proof.
  intros. destruct def.
  destruct f.
  - cbn. auto.
  ...
  - cbn. unfold is_var_internal. 
    ...
    destruct (gvar_readonly v); cbn; auto.
    destruct i; cbn; auto.
    destruct l; cbn; auto.
    destruct i; cbn; auto.
    destruct l; cbn; auto.
...
Lemma get_symbentry_id : forall rd_id d_id c_id rdsz dsz csz id def,
    symbentry_id (get_symbentry rd_id d_id c_id rdsz dsz csz id def) = id.
Proof.
  intros until def.
  destruct def. destruct f.
  simpl; auto.
  simpl; auto.
  simpl. destruct (gvar_init v); destruct (gvar_readonly v); auto.
  destruct i; auto. destruct l; auto.
  destruct i; auto. destruct l; auto.  
Qed.


Lemma get_symbentry_eq_id_inv: 
  forall rdid1 did1 cid1 rdsz1 dsz1 csz1 id1 def1 rdid2 did2 cid2 rdsz2 dsz2 csz2 id2 def2,
    get_symbentry rdid1 did1 cid1 rdsz1 dsz1 csz1 id1 def1 = get_symbentry rdid2 did2 cid2 rdsz2 dsz2 csz2 id2 def2 -> id1 = id2.
Proof.
  intros. 
  generalize (get_symbentry_id rdid1 did1 cid1 rdsz1 dsz1 csz1 id1 def1).
  intros ID1.
  generalize (get_symbentry_id rdid2 did2 cid2 rdsz2 dsz2 csz2 id2 def2).
  intros ID2.
  rewrite H in ID1.
  rewrite ID1 in ID2.
  inv ID2. auto.
...
Lemma get_var_entry_size : forall rdid did cid rdsz dsz csz id v, 
    symbentry_size (get_symbentry rdid did cid rdsz dsz csz id (Gvar v)) = AST.init_data_list_size (gvar_init v).
Proof.
  intros.
  cbn. destruct (gvar_init v); destruct (gvar_readonly v); auto.
  destruct i; auto.
  destruct l; auto.
  cbn. omega.
  destruct i; auto.
...
Lemma acc_init_rodata_app : forall def l1 l2,
    (acc_init_rodata def l1) ++ l2 = acc_init_rodata def (l1 ++ l2).
Proof.
  intros def l1 l2. destruct def as (id & def').
  simpl. rewrite app_assoc. auto.
Qed.

Lemma fold_right_acc_init_rodata_app' : forall defs l,
    fold_right acc_init_rodata [] defs ++ l = fold_right acc_init_rodata l defs.
Proof.
  induction defs. 
  - intros l. simpl. auto.
  - intros l. simpl. 
    rewrite acc_init_rodata_app. rewrite IHdefs. auto.
Qed.

Lemma fold_right_acc_init_rodata_app : forall defs1 defs2,
    fold_right acc_init_rodata [] (defs1 ++ defs2) = 
    fold_right acc_init_rodata [] defs1 ++ fold_right acc_init_rodata [] defs2.
Proof.
  intros.
  rewrite fold_right_app.
  rewrite <- fold_right_acc_init_rodata_app'. auto.
Qed.
...
Lemma update_section_size_eq : forall id def rdsz dsz csz rdincr dincr cincr,
    rdincr = init_data_list_size (get_def_init_rodata def) ->
    dincr = init_data_list_size (get_def_init_data def) ->
    cincr = code_size (get_def_instrs def) ->
    update_section_size rdsz dsz csz id def = (rdincr + rdsz, dincr + dsz, cincr + csz).
Proof.
  intros id def rdsz dsz csz rdincr dincr cincr RDINCR DINCR CINCR.
  unfold update_section_size. destruct def.  destruct f.
  - simpl in *. subst. f_equal; omega.
  - simpl in *. subst. f_equal; omega.
  - simpl in RDINCR, DINCR, CINCR. subst.
    destruct (gvar_init v); destruct (gvar_readonly v);
      try(simpl; auto); try (destruct i; try (f_equal; omega);
      destruct l; f_equal; rewrite Z.add_comm; auto).
Qed.

Lemma acc_symb_size: forall rd_id d_id c_id defs s1 s2 rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 rodata data code,
    fold_left (acc_symb rd_id d_id c_id) defs (s1, rdsz1, dsz1, csz1) = (s2, rdsz2, dsz2, csz2) ->
    init_data_list_size rodata = rdsz1 ->
    init_data_list_size data = dsz1 ->
    code_size code = csz1 ->
    init_data_list_size (fold_right acc_init_rodata rodata defs) = rdsz2 /\
...
Proof.
  ...
  - intros s1 s2 rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 rodata data code SYMB INITRD INITD CODE.
  ...
  - intros s1 s2 rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 rodata data code SYMB INITRD INITD CODE.
    ...
    erewrite update_section_size_eq in Heqp; eauto.
    inv Heqp.
    rewrite <- init_data_list_size_app in SYMB.
    rewrite <- init_data_list_size_app in SYMB.
    rewrite <- code_size_app in SYMB.
    generalize (IHdefs _ _ _ _ _ _ _ _ _ _ _ SYMB
                       (@eq_refl _ (init_data_list_size (get_def_init_rodata def ++ rodata)))
                       (@eq_refl _ (init_data_list_size (get_def_init_data def ++ data)))
                       (@eq_refl _ (code_size (get_def_instrs def ++ code)))).
    destruct 1. destruct H0. subst. split.
    + rewrite <- (fold_right_acc_init_rodata_app' defs rodata).
      rewrite <- (fold_right_acc_init_rodata_app' defs (get_def_init_rodata def ++ rodata)).
      repeat rewrite init_data_list_size_app.
      omega.
    + split. subst. rewrite <- (fold_right_acc_init_data_app' defs data).
...
Lemma gen_symb_table_size: forall defs rd_id d_id c_id stbl rdsz dsz csz,
    gen_symb_table rd_id d_id c_id defs = (stbl, rdsz, dsz, csz) ->
    sec_size (create_rodata_section defs) = rdsz /\
    sec_size (create_data_section defs) = dsz /\
    sec_size (create_code_section defs) = csz.
Proof.
  intros defs rd_id d_id c_id stbl rdsz dsz csz SGEN.
  simpl. unfold gen_symb_table in SGEN.
  destr_in SGEN. destruct p. destruct p. inv SGEN. 
  eapply acc_symb_size; eauto. 
Qed.

Lemma acc_symb_append : forall rd_id r_id c_id defs rdsz1 dsz1 csz1 stbl1 rdsz2 dsz2 csz2 stbl2 stbl3,
    fold_left (acc_symb rd_id r_id c_id) defs (stbl1, rdsz1, dsz1, csz1) = (stbl2, rdsz2, dsz2, csz2) ->
    fold_left (acc_symb rd_id r_id c_id) defs (stbl1 ++ stbl3, rdsz1, dsz1, csz1) = (stbl2 ++ stbl3, rdsz2, dsz2, csz2).
Proof.
  induction defs.
  - intros until stbl2. intros stbl3 ACC. simpl in *.
    inv ACC. auto.
  - intros until stbl2. intros stbl3 ACC. simpl in *.
    destr_in ACC. destruct a. inv Heqp. destr_in ACC. destruct p.
    rewrite app_comm_cons.
    apply IHdefs. auto.
Qed.

Lemma acc_symb_append_nil : forall rd_id r_id c_id defs rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 stbl2 stbl3,
    fold_left (acc_symb rd_id r_id c_id) defs ([], rdsz1, dsz1, csz1) = (stbl2, rdsz2, dsz2, csz2) ->
    fold_left (acc_symb rd_id r_id c_id) defs (stbl3, rdsz1, dsz1, csz1) = (stbl2 ++ stbl3, rdsz2, dsz2, csz2).
Proof.
...
Lemma acc_symb_inv: forall asf rd_id r_id c_id defs stbl1 rdsz1 dsz1 csz1 stbl2 rdsz2 dsz2 csz2,
    asf = (acc_symb rd_id r_id c_id) ->
    fold_left asf defs (stbl1, rdsz1, dsz1, csz1) = (stbl2, rdsz2, dsz2, csz2) ->
    exists stbl1', stbl2 = stbl1' ++ stbl1 /\
              fold_left asf defs ([], rdsz1, dsz1, csz1) = (stbl1', rdsz2, dsz2, csz2).
Proof.
  induction defs.
  - intros stbl1 rdsz1 dsz1 csz1 stbl2 rdsz2 dsz2 csz2 ASF ACC.
  ...
  - intros until csz2. intros ASF ACC. simpl in *.
    ...
    destruct (update_section_size rdsz1 dsz1 csz1 id def) as [dsize' csize'] eqn:UPDATE. destruct dsize'.
    rewrite <- ASF in ACC.
    generalize (IHdefs _ _ _ _ _ _ _ _ ASF ACC).
...
Lemma acc_symb_inv': forall rd_id r_id c_id defs stbl1 rdsz1 dsz1 csz1 stbl2 rdsz2 dsz2 csz2,
    fold_left (acc_symb rd_id r_id c_id) defs (stbl1, rdsz1, dsz1, csz1) = (stbl2, rdsz2, dsz2, csz2) ->
    exists stbl1', stbl2 = stbl1' ++ stbl1 /\
              fold_left (acc_symb rd_id r_id c_id) defs ([], rdsz1, dsz1, csz1) = (stbl1', rdsz2, dsz2, csz2).
Proof.
...
Lemma get_symbentry_in_range: forall rd_id r_id c_id rdsz dsz csz id def,
  symbentry_index_in_range [rd_id; r_id; c_id] (get_symbentry rd_id r_id c_id rdsz dsz csz id def).
Proof.
  intros.
  destruct def. destruct f.
  - cbn. auto.
  - cbn. auto.
  - cbn. destr; cbn; auto. destruct (gvar_readonly v);
    destruct i; cbn; auto.
    destruct l; cbn; auto.
    destruct l; cbn; auto.
Qed.

Lemma acc_symb_index_in_range: forall rd_id r_id c_id defs rdsz1 dsz1 csz1 tbl rdsz2 dsz2 csz2,
      fold_left (acc_symb rd_id r_id c_id) defs ([], rdsz1, dsz1, csz1) = (tbl, rdsz2, dsz2, csz2)
      -> symbtable_indexes_in_range [rd_id; r_id; c_id] (rev tbl).
Proof.
  induction defs as [| def defs].
  - intros until csz2. intros ACC. cbn in *.
    inv ACC. red. cbn. apply Forall_nil.
  - intros until csz2. intros ACC. cbn in *.
    destruct def as [id def]. destr_in ACC. destruct p.
    generalize (acc_symb_inv _ _ _ _ _ _ _ _ _ _ _ _ _ eq_refl ACC).
    destruct 1 as (stbl1' & EQ & ACC'). subst.
    rewrite rev_unit.
    generalize (IHdefs _ _ _ _ _ _ _ ACC').
    intros IN_RNG.
...
Lemma gen_symb_table_index_in_range : forall defs sec_rodata_id sec_data_id sec_code_id stbl rdsz dsz csz,
    gen_symb_table sec_rodata_id sec_data_id sec_code_id defs = (stbl, rdsz, dsz, csz) ->
    symbtable_indexes_in_range [sec_rodata_id; sec_data_id; sec_code_id] stbl.
Proof.
  intros until csz.
  intros GS.
  unfold gen_symb_table in GS. destr_in GS. destruct p. destruct p. inv GS.
...
Lemma get_var_entry_type : forall rodid did cid rdsz1 dsz1 csz1 id v,
    symbentry_type (get_symbentry rodid did cid rdsz1 dsz1 csz1 id (Gvar v)) = symb_data.
Proof.
  intros. cbn.
  destruct (gvar_init v); destruct (gvar_readonly v); auto.
  destruct i; auto.
  destruct l; auto.
  destruct i; auto.
  destruct l; auto.
...
Lemma get_fun_entry_type : forall rodid did cid rdsz1 dsz1 csz1 id f,
    symbentry_type (get_symbentry rodid did cid rdsz1 dsz1 csz1 id (Gfun f)) = symb_func.
Proof.
  intros. cbn.
  destruct f; auto.
Qed.
...
(* SACC 
Lemma get_none_entry_type : forall rodid did cid rdsz1 dsz1 csz1 id,
    symbentry_type (get_symbentry rodid did cid rdsz1 dsz1 csz1 id None) = symb_notype.
Proof.
  intros. cbn. auto.
Qed.

Lemma get_none_entry_secindex : forall rodid did cid rdsz1 dsz1 csz1 id,
    symbentry_secindex (get_symbentry rodid did cid rdsz1 dsz1 csz1 id None) = secindex_undef.
Proof.
  intros. cbn. auto.
Qed.
*)

Lemma get_symbentry_eq_gvar_init_readonly: forall rodid did cid rdsz1 dsz1 csz1 id v1 v2,
    gvar_init v1 = gvar_init v2 ->
    gvar_readonly v1 = gvar_readonly v2 ->
    get_symbentry rodid did cid rdsz1 dsz1 csz1 id (Gvar v1) = 
    get_symbentry rodid did cid rdsz1 dsz1 csz1 id (Gvar v2).
Proof.
  intros. cbn. rewrite H. rewrite H0. auto.
Qed.

Lemma update_size_offset : forall id def rdsz dsz csz rdsz1 dsz1 csz1 rdofs dofs cofs,
    update_section_size rdsz dsz csz id def = (rdsz1, dsz1, csz1) ->
    update_section_size (rdofs + rdsz) (dofs + dsz) (cofs + csz) id def = (rdofs + rdsz1, dofs + dsz1, cofs + csz1).
Proof.
  intros until cofs. intros UPDATE.
  destruct def. destruct f.
  - cbn in *. inv UPDATE. f_equal; omega.
  - cbn in *. inv UPDATE. f_equal; omega.
  - unfold update_section_size in *.
    destruct (gvar_init v); destruct (gvar_readonly v).
    inv UPDATE. reflexivity.
    inv UPDATE. reflexivity.
    destruct i; destruct l;
      try (inv UPDATE; cbn; rewrite <- Z.add_assoc; reflexivity).    
    inv UPDATE. reflexivity.
    destruct i; destruct l;
      try (inv UPDATE; cbn; rewrite <- Z.add_assoc; reflexivity).    
    inv UPDATE. reflexivity.
...
Lemma acc_symb_pres_ids: forall rodid did cid defs rdsz1 dsz1 csz1 stbl2 rdsz2 dsz2 csz2,
    fold_left (acc_symb rodid did cid) defs ([], rdsz1, dsz1, csz1) = (stbl2, rdsz2, dsz2, csz2) ->
    (map fst defs) = get_symbentry_ids (rev stbl2).
Proof.
  induction defs as [| def defs].
  - cbn.  inversion 1. subst. inv H.
    cbn. auto.
  - intros rdsz1 dsz1 csz1 stbl2 rdsz2 dsz2 csz2 FL.
    cbn in *. destruct def as (id, def).
    destr_in FL. destruct p. cbn.
...
Lemma gen_symb_table_pres_ids: forall rdid did cid defs stbl rdsz dsz csz,
    gen_symb_table rdid did cid defs = (stbl, rdsz, dsz, csz) ->
    (map fst defs) = (get_symbentry_ids stbl).
Proof.
  intros rdid did cid defs stbl rdsz dsz csz GST.
  unfold gen_symb_table in GST.
  destr_in GST. destruct p. destruct p. inv GST.
  apply acc_symb_inv' in Heqp.
...
Definition defs_before {F V}
           (id:ident) (defs: list (ident * (globdef F V))) :=
...
Lemma defs_before_tail:
  forall {F V} id id' (def : (globdef F V)) defs,
  id <> id' ->
...
Lemma defs_before_head:
  forall {F V} id  (def : (globdef F V)) defs,
...
Definition def_rodata_size (def: gdef) :=
  init_data_list_size (get_def_init_rodata def).

Definition def_data_size (def: gdef) :=
  init_data_list_size (get_def_init_data def).

Definition def_code_size (def: gdef) :=
  code_size (get_def_instrs def).

Definition defs_rodata_size (defs: list gdef) :=
  fold_right (fun def sz => def_rodata_size def + sz) 0 defs.
             
Definition defs_data_size (defs: list gdef) :=
  fold_right (fun def sz => def_data_size def + sz) 0 defs.

Definition defs_code_size (defs: list gdef) :=
  fold_right (fun def sz => def_code_size def + sz) 0 defs.

Lemma defs_rodata_size_cons: forall def defs,
      defs_rodata_size (def :: defs) = (def_rodata_size def) + defs_rodata_size defs.
Proof.
  intros. unfold defs_rodata_size.
  cbn. auto.
Qed.
...
Lemma symbtable_to_tree_cons: forall rdid did cid tl rdsz1 dsz1 csz1 id def,
    ~In id (get_symbentry_ids tl) ->
    (symbtable_to_tree (get_symbentry rdid did cid rdsz1 dsz1 csz1 id def :: tl)) ! id = Some (get_symbentry rdid did cid rdsz1 dsz1 csz1 id def).
Proof.
  intros rdid did cid tl rdsz1 dsz1 csz1 id def IN.
...
Lemma symbtable_to_tree_tail: forall rdid did cid tl rdsz1 dsz1 csz1 id id' def,
    id <> id' ->
    (symbtable_to_tree (get_symbentry rdid did cid rdsz1 dsz1 csz1 id def :: tl)) ! id' =
...
Lemma update_section_size_inv: forall rdsz1 dsz1 csz1 id def rdsz2 dsz2 csz2,
    update_section_size rdsz1 dsz1 csz1 id def = (rdsz2, dsz2, csz2) ->
    rdsz2 = rdsz1 + def_rodata_size def /\
    dsz2 = dsz1 + def_data_size def /\
    csz2 = csz1 + def_code_size def.
Proof.
  intros rdsz1 dsz1 csz1 id def rdsz2 dsz2 csz2 UPD.
  unfold update_section_size in UPD. 
  destruct def. destruct f.
  - unfold def_rodata_size, def_data_size, def_code_size.
    simpl in *. inv UPD. f_equal; omega.
  - unfold def_rodata_size, def_data_size, def_code_size.
    inv UPD. cbn. split; omega.
  - destruct (gvar_init v) eqn:V0;destruct (gvar_readonly v) eqn:V1.
    + inv UPD. cbn. rewrite V0. cbn. split; omega.
    + inv UPD. cbn. rewrite V0. cbn. split; omega.
    + destruct i; try (inv UPD; cbn; rewrite V0; rewrite V1; cbn; split; omega).
      destruct l; try (inv UPD; cbn; rewrite V0; rewrite V1; cbn; split; omega).
      inv UPD. cbn. rewrite V0. cbn. split. omega. omega.
    + destruct i; try (inv UPD; cbn; rewrite V0; rewrite V1; cbn; split; omega).
      destruct l; try (inv UPD; cbn; rewrite V0; rewrite V1; cbn; split; omega).
      inv UPD. cbn. rewrite V0. cbn. split. omega. omega.
Qed.

Lemma acc_symb_tree_entry_some : forall rdid did cid defs rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 stbl id def,
    list_norepet (map fst defs) ->
    fold_left (acc_symb rdid did cid) defs ([], rdsz1, dsz1, csz1) = (stbl, rdsz2, dsz2, csz2) ->
    (PTree_Properties.of_list defs)!id = Some def ->
    (symbtable_to_tree (rev stbl))!id 
    = Some (get_symbentry rdid did cid
             (rdsz1 + defs_rodata_size (defs_before id defs))           
...
Proof.
  ...
  - intros rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 stbl id def NORPT ACC GET.
    cbn in ACC. destruct def' as (id', def').
    destr_in ACC. destruct p. apply acc_symb_inv' in ACC.
    ...
    + assert ((PTree_Properties.of_list defs) ! id = Some def) as GET'.
      ...
      inv NORPT.
      generalize (IHdefs _ _ _ _ _ _ _ _ _ H2 ACC GET').
      intros SEQ. rewrite SEQ.
      setoid_rewrite defs_before_tail; auto.
      cbn [defs_data_size defs_code_size].
      rewrite defs_rodata_size_cons.
      rewrite defs_data_size_cons.
      rewrite defs_code_size_cons.
      generalize (update_section_size_inv _ _ _ _ _ _ _ _ Heqp).
      intros (EQ1 & EQ2); subst.
      f_equal. f_equal; omega.
Qed.


Lemma acc_symb_tree_entry_none : forall rdid did cid defs rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 stbl id,
    fold_left (acc_symb rdid did cid) defs ([], rdsz1, dsz1, csz1) = (stbl, rdsz2, dsz2, csz2) ->
    (PTree_Properties.of_list defs)!id = None ->
    (symbtable_to_tree (rev stbl))!id = None.
Proof.
  induction defs as [|def defs].
  - cbn. intros rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 stbl id EQ GET.
    inv EQ. cbn.
    rewrite PTree.gempty. auto.
  - intros rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 stbl id FL GET.
    destruct def as (id', def').
    cbn in FL.
    destr_in FL. destruct p.
    apply acc_symb_inv' in FL.
...
Lemma acc_symb_tree_entry_some_inv:
  forall (rdid did cid : N) (defs : list (ident * (globdef fundef unit)))
    (rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 : Z) (stbl : symbtable) (id : positive) e,
  list_norepet (map fst defs) ->
  fold_left (acc_symb rdid did cid) defs ([], rdsz1, dsz1, csz1) = (stbl, rdsz2, dsz2, csz2) ->
...
Proof.
  ...
  - intros until e.
    ...
    + erewrite symbtable_to_tree_tail in ST; eauto.
      generalize (IHdefs _ _ _ _ _ _ _ _ _ H2 ACC ST).
      intros (def1 & GET1).
      rewrite PTree_Properties_of_list_tail; eauto.
Qed.

Definition symbtable_entry_equiv_sizes stbl rdsz1 dsz1 csz1 defs1 :=
  forall rdid did cid rdsz3 dsz3 csz3 id def,
    In (get_symbentry rdid did cid rdsz3 dsz3 csz3 id def) stbl ->
    get_symbentry rdid did cid rdsz3 dsz3 csz3 id def =
    get_symbentry rdid did cid
                  (rdsz1 + defs_rodata_size (defs_before id defs1))
                  (dsz1 + defs_data_size (defs_before id defs1))
                  (csz1 + defs_code_size (defs_before id defs1))
                  id def.

Lemma symbtable_entry_equiv_sizes_app_comm: forall l1 l2 rdsz dsz csz defs,
    symbtable_entry_equiv_sizes (l1 ++ l2) rdsz dsz csz defs ->
    symbtable_entry_equiv_sizes (l2 ++ l1) rdsz dsz csz defs.
Proof.
  intros l1 l2 rdsz dsz csz defs SE.
...
Lemma symbtable_entry_equiv_sizes_app: forall l1 l2 rdsz dsz csz defs,
    symbtable_entry_equiv_sizes l1 rdsz dsz csz defs ->
    symbtable_entry_equiv_sizes l2 rdsz dsz csz defs ->
    symbtable_entry_equiv_sizes (l1 ++ l2) rdsz dsz csz defs.
Proof.
  intros l1 l2 rdsz dsz csz defs SE1 SE2.
  red. intros rdid did cid rdsz3 dsz3 csz3 id def IN.
...
Lemma acc_symb_size':
  forall (rd_id r_id c_id : N) (defs : list (ident * (globdef fundef unit)))
    (s1 s2 : symbtable) (rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 : Z),
    fold_left (acc_symb rd_id r_id c_id) defs (s1, rdsz1, dsz1, csz1) = (s2, rdsz2, dsz2, csz2) ->
    rdsz2 = rdsz1 + defs_rodata_size (map snd defs) /\
    dsz2 = dsz1 + defs_data_size (map snd defs) /\
    csz2 = csz1 + defs_code_size (map snd defs).
Proof.
  induction defs as [|def defs].
  - cbn. inversion 1. split; omega.
  - intros s1 s2 rdsz1 dsz1 csz1 rdsz2 dsz2 csz2 ACC.
    cbn in ACC. destruct def as (id, def). 
    cbn [map snd].
    rewrite defs_code_size_cons.
    rewrite defs_data_size_cons.
    ...
    destr_in ACC. destruct p.
    generalize (update_section_size_inv _ _ _ _ _ _ _ _ Heqp).
    intros (EQ1 & EQ2 & EQ3). subst.
    apply IHdefs in ACC. 
    destruct ACC. destruct H0.
    subst; omega.
Qed.
...
Lemma code_size_bound: forall defs (id:ident) f,
    In (id, (Gfun (Internal f))) defs ->
    code_size (fn_code f) <= defs_size (map snd defs).
Proof.
  induction defs as [|def defs].
  - cbn. intros. contradiction.
  - cbn. intros id f [EQ | IN].
    + subst. cbn. 
      generalize (defs_size_pos (map snd defs)).
      intros LE.
...
Lemma def_code_size_le_def_size : forall def, 
    def_code_size def <= def_size def.
Proof.
  intros. destruct def. destruct f.
  cbn. omega.
  cbn. omega.
  cbn. generalize (init_data_list_size_pos (gvar_init v)). omega.
Qed.

Lemma pres_find_instr_aux: forall (defs: list (ident * (globdef fundef unit))) id f ofs i cz t ofs' t',
    list_norepet (map fst defs) ->
    In (id, (Gfun (Internal f))) defs ->
...
Proof.
  ...
  - intros id f ofs i cz t ofs' t' NORPT IN FIND ACC.
    assert (Ptrofs.unsigned ofs' + defs_size (map snd (def::defs)) <= Ptrofs.max_unsigned) as SZ. 
    { admit. }
    inv NORPT.
    ...
    + destruct def as (id', def).
      ...
      assert (def_code_size def <= def_size def) as DCBND.
      { eapply def_code_size_le_def_size. }
...
Lemma gen_symb_table_only_internal_symbol_aux: 
  forall rdid did cid defs stbl rdz1 dz1 cz1 rdz2 dz2 cz2 id def,
    list_norepet (map fst defs) ->
    is_def_internal is_fundef_internal def = true ->
    fold_left (acc_symb rdid did cid) defs ([], rdz1, dz1, cz1) = (stbl, rdz2, dz2, cz2) ->
    In (id, def) defs ->
    only_internal_symbol id (rev stbl).
Proof.
  induction defs as [|def defs].
  - cbn. intros. contradiction.
  - intros stbl rdz1 dz1 cz1 rdz2 dz2 cz2 id def1 NORPT INT ACC IN. 
...
Lemma gen_symb_table_only_internal_symbol: 
  forall rdid did cid defs stbl rdz dz cz id def,
    list_norepet (map fst defs) ->
    is_def_internal is_fundef_internal def = true ->
    gen_symb_table rdid did cid defs = (stbl, rdz, dz, cz) ->
    In (id, def) defs ->
...
Proof.
  ...
  - cbn. intros sz IAL SAL AL.
    ...
    specialize (IAL (Gvar v) (in_eq _ _)).
    specialize (SAL (Gvar v) (in_eq _ _)).
    cbn in IAL, SAL.
    rewrite Heql in IAL.
