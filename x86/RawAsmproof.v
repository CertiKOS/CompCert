...
Section WITHMEMORYMODEL.
  ...
  Lemma no_stack_no_nil m:
    ...
    Mem.stack m <> nil.
  Proof.
    intros (frstack & fstack & r & EQstk & BLOCKS & ZERO & SZ).
  ...
  | match_states_intro:
      forall j (rs: regset) (m: mem) (rs': regset) m' g
        ...
        (SPinstack: in_stack' (Mem.stack m) (binit_sp, default_frame_info 0)),
...
Definition cmpu_bool_inject := fun j g m m' (MINJ: Mem.inject j g m m') =>
                     Val.cmpu_bool_inject j (Mem.valid_pointer m) (Mem.valid_pointer m')
                                          (fun b1 ofs b2 delta FB => valid_pointer_inject' j g m m' b1 ofs b2 delta FB MINJ)
                                          (fun b1 ofs b2 delta FB => weak_valid_pointer_inject' j g m m' b1 ofs b2 delta FB MINJ)
...
(***** Remove Proofs By Chris End ******)
  ...
  Lemma alloc_global_eq:
    forall F V (l: (ident * (globdef F V))) m (ge: _ F V) id gv,
      (forall v, snd l = (Gvar v) -> Forall (fun x => init_data_diff x id) (gvar_init v)) ->
  ...
  Lemma alloc_globals_eq:
    forall F V (l: list (ident * (globdef F V))) m (ge: _ F V) id gv,
      (forall x v, In x l -> snd x = (Gvar v) -> Forall (fun x => init_data_diff x id) (gvar_init v)) ->
  ...
  Proof.
                          ...
                          (flat_frameinj (length (Mem.stack (Mem.push_new_stage m0)))) m2 m4').
    {
      ...
      - red; intros.
        ...
        eexists; split; eauto.
        split; simpl; intros; simpl in H0;
        rewrite Z.max_r in H0; omega.
