...
Require Import AST_old Integers Floats Memdata_old.
Require Import Op_old Locations_old Mach_old Asm_old.
...
Definition transl_instr (f: Mach_old.function) (i: Mach_old.instruction)
  ...
  | Mgetparam ofs ty dst =>
      if ax_is_parent then
        ...
        (do k1 <- loadind RAX ofs ty dst k;
           OK ((Pload_parent_pointer RAX (Mach_old.fn_stacksize f)) :: k1)
  ...
  | Mtailcall sig (inl reg) =>
      do r <- ireg_of reg;
      OK ((Pfreeframe (Mach_old.fn_stacksize f) f.(fn_retaddr_ofs)) ::
          (Pjmp (inl r) sig) :: k)
...
Definition it1_is_parent (before: bool) (i: Mach_old.instruction) : bool :=
  match i with
...
Fixpoint transl_code (f: Mach_old.function) (il: list Mach_old.instruction) (axp: bool) :=
  match il with
  | nil => OK nil
  | i1 :: il' =>
...
Fixpoint transl_code_rec (f: Mach_old.function) (il: list Mach_old.instruction)
...
Definition transl_code' (f: Mach_old.function) (il: list Mach_old.instruction) (axp: bool) :=
...
Definition transl_function (f: Mach_old.function) :=
  do c <- transl_code' f f.(Mach_old.fn_code) true;
    match StackADT_old.frame_info_of_size_and_pubrange (Mach_old.fn_stacksize f) (Mach_old.fn_frame_pubrange f) with
      Some fi =>
      OK (mkfunction (Mach_old.fn_sig f)
                     ((Pallocframe (Mach_old.fn_stacksize f) (Mach_old.fn_frame_pubrange f) (fn_retaddr_ofs f)) :: c)
                     (Mach_old.fn_stacksize f) (Mach_old.fn_frame_pubrange f))
    | None => Error (msg "should not happen -- Mach_old.fn_stacksize <= 0")
    end.

Definition transf_function (f: Mach_old.function) : res Asm_old.function :=
  do tf <- transl_function f;
  if zlt Ptrofs.max_unsigned (code_size tf.(fn_code))
  then Error (msg "code size exceeded")
  else OK tf.

Definition transf_fundef (f: Mach_old.fundef) : res Asm_old.fundef :=
  transf_partial_fundef transf_function f.

Definition transf_program (p: Mach_old.program) : res Asm_old.program :=
