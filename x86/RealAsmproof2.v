...
Lemma stack_limit_pos: 0 < Mem.stack_limit.
Proof.
  unfold Mem.stack_limit.
...
Qed.
  ...
  Proof.
    split.
    generalize Mem.stack_limit_aligned stack_limit_pos. intros (x & EQ) POS. rewrite EQ.
    transitivity 8.
  ...
  Proof.
    ...
    assert (encode_val Mptr (rs1 RA) = l).
    {
      ...
      eapply encode_decode_long; eauto. eapply Mem.loadbytes_length in Heqo. rewrite length_inj_bytes in Heqo. rewrite Heqo.
      unfold Mptr; rewrite Heqb0; reflexivity.
      apply inj_proj_bytes in Heqo0. subst. f_equal.
      eapply encode_decode_int; eauto. eapply Mem.loadbytes_length in Heqo. rewrite length_inj_bytes in Heqo. rewrite Heqo.
      unfold Mptr; rewrite Heqb0; reflexivity.
    ...
    subst.          
    exploit store_same_ptr; eauto. intro; subst. auto.
  Qed.
  ...
  Proof.
    ...
    - simpl in PC1. repeat destr_in PC1.
      ...
      }
      {
        ...
        rewrite RNG.
        intros [EQ|NOOV]. subst; inv IFREE.
        generalize (instr_size_positive (Pallocframe sz ora)) (instr_size_positive ifree). omega.
      }
    - simpl in PC1. repeat destr_in PC1.
      rewrite REQ in Heqv by congruence.
      inv STEP; rewrite_hyps.
