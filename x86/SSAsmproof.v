Require Import Coqlib Ourlib Errors.
...
Record single_stack_perm_ofs (m: mem) : Prop :=
  ...
  stack_offset_perm: forall (ofs: Z) (k: perm_kind) (p: permission),
      0 <= ofs < Mem.max_stacksize -> Mem.perm m stkblock ofs k p;
  }.

Lemma stack_size_aligned :
  forall stk,
    (8 | Mem.stack_size stk).
Proof.
  ...
  - induction a; simpl.
    + auto.
    + simpl in IHa.
      generalize (Z.add_comm (Mem.size_of_all_frames a0) (Mem.stack_size stk)). intro. rewrite H in IHa.
      exploit Z.divide_add_cancel_r. apply IHstk. apply IHa. intro.
      assert (8 | align (Mem.frame_size a) 8). apply align_divides. lia.
...
| match_states_intro:
    forall (j: meminj)
      ...
      (RSPINJ': exists stkofs, (Ptrofs.unsigned stkofs = Mem.max_stacksize - Mem.stack_size (Mem.stack (Mem.support m))) /\ rs' RSP = Vptr stkblock stkofs)
      (STKINJLWBD: stack_inject_lowbound j m (Mem.max_stacksize - Mem.stack_size (Mem.stack (Mem.support m)))),
      (* (STKINJ: inject_stack j (Mem.stack (Mem.support m))), *)
...
Lemma goto_label_inject:
  forall j rs1 rs2 m1 m2
    ...
    l f rs1' m1',
    eAsm.goto_label ge f l rs1 m1 = Next rs1' m1' ->
    exists rs2' m2',
      eAsm.goto_label ge f l rs2 m2 = Next rs2' m2' /\
      Mem.inject j m1' m2' /\
      forall r, Val.inject j (rs1' r) (rs2' r).
Proof.
...
Lemma exec_instr_inject_normal:
  forall j m1 m2 rs1 rs2 f i rs1' m1'
    ...
    (EI: eAsm.exec_instr ge f i rs1 m1 = Next rs1' m1'), 
  exists rs2' m2',
    eAsm.exec_instr ge f i rs2 m2 = Next rs2' m2'
    /\ Mem.inject j m1' m2'
    /\ (forall r, Val.inject j (rs1' # r) (rs2' # r)).
Proof.
  intros.
...
Lemma allocframe_inject:
  forall j stkb m m1 m2 m3 m4 m' rs rs' sz ofs_link ofs_ra
    (MS: match_states j (State rs m) (State rs' m'))
    (ALLOC: Mem.alloc m 0 sz = (m1, stkb))
    (SIZE: sz > 0)
    (RECORDFR: Mem.record_frame (Mem.push_stage m1) (Mem.mk_frame sz) = Some m2)
...
Proof.
  ...
  remember (Mem.mk_frame sz) as fr.
  assert (FRSZ: sz = Mem.frame_size fr). rewrite Heqfr. simpl. lia.  
  assert (OLDSTKSIZE: aligned_sz <= stksize <= Ptrofs.max_unsigned).
  {
    ...
    - rewrite Heqstkofs. apply Z.le_add_le_sub_r.
      ...
      simpl in H. exploit Mem.stack_alloc. apply ALLOC. intros ALLOCSTK.
      rewrite <- ALLOCSTK. auto.
  ...
  trim A.
  {
    ...
    split. rewrite Heqstksize'. omega.
    rewrite Heqstksize'. rewrite Heqstksize.
    eapply Z.lt_le_trans with stksize. omega.
    rewrite Heqstkofs.
    generalize (Mem.stack_size_pos (Mem.stack (Mem.support m))).
    intros STKSZPOS. omega.
  }
  trim A.
  {
    red. intros. subst.
    ...
    generalize (stack_size_aligned (Mem.stack (Mem.support m))).
    intro ALI1.
    assert (ALI2: (8 | Mem.max_stacksize)). unfold Mem.max_stacksize. red. exists 512. omega.
    rewrite Heqstkofs.
  ...
  (* store link *)
  assert (ALIGNED_SZ: 0 <= align sz 8 <= Ptrofs.max_unsigned). omega.
          ...
          Ptrofs.unsigned OFS + STKSIZE) as EQ'.
  {
    ...
    rewrite HeqOFS. rewrite Ptrofs.unsigned_repr; generalize (size_chunk_pos Mptr); omega.
  }
  ...
  assert (SUPINCALL: Mem.sup_include (Mem.support m) (Mem.support m4)).
  {
    ...
    intros.
    eapply Mem.sup_include_trans. apply H3.
    eapply Mem.sup_include_trans. apply H2.
    eapply Mem.sup_include_trans. apply H1.
    rewrite H,H0. apply Mem.sup_include_refl.
  }
  assert (SUPINCALL': Mem.sup_include (Mem.support m') (Mem.support m4')).
  {
    generalize (Mem.support_store _ _ _ _ _ _ STORELINK').
  ...
  assert(STKOFS': Ptrofs.unsigned stkofs' = Mem.max_stacksize - Mem.stack_size (Mem.stack (Mem.support m4))).
  {
    generalize (Mem.support_store _ _ _ _ _ _ STORELINK).
    intro SUPSTLINK.
    generalize (Mem.support_store _ _ _ _ _ _ STORERA).
    ...
    generalize (Mem.stack_record_frame _ _ _ RECORDFR).
    ...
    generalize (Mem.support_alloc _ _ _ _ _ ALLOC).
    intro SUPAL. rewrite SUPAL in H3. simpl in H3.
    rewrite <- H3 in H2.
    rewrite H2.
    assert (Mem.stack_size ((fr :: nil) :: Mem.stack (Mem.support m)) = align (Mem.frame_size fr) 8 + Mem.stack_size (Mem.stack (Mem.support m))). simpl. omega.
  ...
  - red. intros b0 delta o k p JB PERM.
    ...
    * subst b0. rewrite EQ1 in JB.
      ...
      rewrite (Ptrofs.unsigned_repr _ NEWSTKSIZE). omega.
    * rewrite <- STKOFS'.
...
Lemma exec_instr_inject:
  forall j m m' rs rs' f i m1 rs1
    (EI: eAsm.exec_instr ge f i rs m = Next rs1 m1)
    (MS: match_states j (State rs m) (State rs' m'))
...
Proof.
  ...
  - (* builtin step *)
    ...
    exploit (external_call_mem_stackeq ef). apply H3. intros STKEQ.    
    exploit (external_call_mem_stackeq ef). apply EC. intros STKEQ0.
    generalize (globinj_to_funinj j GLOBINJ). intros GLOBFUN_INJ.
    assert (j b = Some (b,0)) as JB.
    {
      eapply GLOBFUN_INJ. eauto.
...
*)
  ...
  - (* external call *)
    ...
    exploit (external_call_mem_stackeq ef). apply H2. intros STKEQ.    
    exploit (external_call_mem_stackeq ef). apply EC. intros STKEQ0.
...
Lemma transf_initial_states:
  forall st1, eAsm.initial_state prog st1 ->
...
Proof.
  ...
  rename H2 into RECORDFR.
  (* exploit Genv.initmem_inject; eauto. intro FLATINJ. *)
  assert (ALLOCINJ: exists (j : meminj) (m1' m2' : mem) (stk' : block),
             Mem.alloc m0 0 Mem.max_stacksize = (m1', stk) /\
             Mem.record_frame (Mem.push_stage m1') (Mem.mk_frame Mem.max_stacksize) = Some m2' /\
             Mem.inject j m0 m2' /\
             inject_incr (Mem.flat_inj (Mem.support m0)) j /\
             j stk = Some (stk', Mem.max_stacksize) /\
...
Theorem transf_program_correct:
  forward_simulation (eAsm.semantics prog) (SSAsm.semantics prog).
Proof.
