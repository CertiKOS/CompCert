...
Lemma get_internal_var_entry : forall rdsec dsec csec rdsz dsz csz id v,
    ...
    get_symbentry rdsec dsec csec rdsz dsz csz id (Gvar v) = 
    {|symbentry_id := id;
...
Lemma link_varinit_internal_external_pres_aligned:
  forall (v1 v2: globvar unit) l inf rd vl,
    ...
    -> def_aligned (Gvar (mkglobvar inf l rd vl)).
Proof.
...
Lemma external_var_aligned: forall v,
    is_var_internal v = false -> def_aligned (Gvar v).
Proof.
  intros v INT.
  unfold is_var_internal in INT.
...
Proof.
  ...
  destruct def2. destruct f; cbn in *; try congruence.
  - destruct def1. destruct f. 
    + cbn in LINK. destr_in LINK; try congruence. inv LINK.
      destr_in Heqo; try congruence; inv Heqo. auto.
    + cbn in LINK. destr_in LINK; try congruence. inv LINK.
  ...
  - destruct def1.
    ...
    + cbn in LINK. destr_in LINK; try congruence.
      destr_in Heqo; try congruence. inv LINK.
      cbn in INT.
...
Proof.
  ...
  - setoid_rewrite link_def_symm in LINK; eauto.
    apply link_option_internal_external_pres_aligned 
