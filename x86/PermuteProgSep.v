...
Lemma prog_defmap_perm: 
  forall {F V} {LF: Linker F} {LV: Linker V}
    ...
    (prog_defmap p1) ! x = (prog_defmap tp1) ! x.
Proof.
  intros.
  unfold prog_defmap.
...
Proof.
  ...
  - repeat rewrite andb_true_iff in CHK. 
    ...
    erewrite <- prog_defmap_perm; eauto.
    rewrite Heqo.
...
Lemma link_prog_check_all_perm : 
  forall {F V} {LF: Linker F} {LV: Linker V}
    ...
    Permutation (prog_defs p1) (prog_defs tp1) ->
    Permutation (prog_defs p2) (prog_defs tp2) ->
    PTree_Properties.for_all (prog_defmap p1)
                             (link_prog_check p1 p2) = true ->
    PTree_Properties.for_all (prog_defmap tp1)
...
Proof.
  ...
  intros x a GET.
  generalize (in_prog_defmap _ _ GET); eauto.
  intros IN.
  apply Permutation_sym in PERM1.
  generalize (Permutation_in _ PERM1 IN).
  intros IN'.
  generalize (prog_defmap_norepet _ _ _ NORPT1 IN').
  intros GET'.
