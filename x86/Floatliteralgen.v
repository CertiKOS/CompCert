...
Definition gen_float_mask :
  (list (ident * option (globdef fundef unit))) * (ident * ident * ident * ident) :=
  ...
  let negd_def := Some (Gvar negd_var) in
  let negs_def := Some (Gvar negs_var) in
  let absd_def := Some (Gvar absd_var) in
  let abss_def := Some (Gvar abss_var) in
...
Definition acc_flt_lit (t: code * list (ident * option (globdef fundef unit)))
           (i: instruction) :=
  let (c, flt_lit) := t in
...
Definition transf_code (c: code) :
  (code*(list (ident * option (globdef fundef unit)))) :=
   fold_left acc_flt_lit c ([],[]).
     
Definition transf_def (iddef: ident * option (AST.globdef Asm.fundef unit))
  : list (ident * option (AST.globdef Asm.fundef unit)) :=
  match iddef with
  | (id, None) => [(id, None)]
  | (id, Some (Gvar v)) => [(id, Some (Gvar v))]
  | (id, Some (Gfun (External ef))) => [(id, Some (Gfun (External ef)))]
  | (id, Some (Gfun (Internal f))) =>
    let c := fn_code f in
    let (c', flt_lit) :=  transf_code c in 
    let f' := mkfunction (fn_sig f) c' (fn_stacksize f) (fn_pubrange f) in
    [(id, Some (Gfun (Internal f')))] ++ flt_lit
  end.
