...
Import Hex Bits.
Require Import RelocBingen2 RelocProgram SeqTable Encode.
...
Section PRESERVATION.
  ...
  Axiom prog: RelocProgram.program.
  
Let relocTables := (prog_reloctables prog).

Let symbolTable := (prog_symbtable prog).

Axiom textRelocTable: reloctable.

Axiom allCode : list byte.
...
Fixpoint transl_code_aux (n:nat) (c:list byte) : res (list Asm.instruction) :=
  match n with
...
Fixpoint transl_code (c:list byte) : res (list Asm.instruction) :=
