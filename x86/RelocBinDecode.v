...
Proof.
  ...
  +
    (* destruct (instr_reloc_offset i); *)
    (* try (destruct (get_instr_reloc_addend' rtbl_ofs_map (z0+sofs)); inversion EQ1). *)
    (* Focus 2. *)
    (* 1,2: *)
    ...
    (*    +++ destruct (ireg_eq i0 RSP); monadInv EQ1. *)
    ...
    (*          rewrite HEQX3. *)
    (*          generalize (encode_decode_ireg_refl _ _  EQ1). *)
    (*          intros HRi. *)
