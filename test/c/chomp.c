...
static inline int equal_data(int *data1,int *data2) /* check if both _data's are equal */
{
  int counter = ncol;
...
static inline int valid_data(int *data) /* checks if the play could ever be achieved. */
{
...
static inline int get_value(int *data) /* get the value (0 or 1) for a specific _data */
{
  struct _play *search;
...
static inline int in_wanted(int *data) /* checks if the current _data is in the wanted list */
{
...
static inline int *get_good_move(struct _list *list) /* gets the first good move from a _list */
{
...
static inline int *get_winning_move(struct _play *play) /* just scans for the first good move */
