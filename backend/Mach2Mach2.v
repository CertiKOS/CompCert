...
Require Import AST Values Memory Globalenvs Events Smallstep.
...
Lemma parent_sp_same_tl:
  forall s1 s2 cs
    (LP1: call_stack_agree init_stk cs (tl s1))
    (LP2: call_stack_agree init_stk cs (tl s2))
...
forall cs s,
  call_stack_agree init_stk cs s ->
  length s = (length init_stk + length cs)%nat.
Proof.
  induction 1; simpl; intros; eauto. subst; omega.
  rewrite IHcall_stack_agree. omega.
Qed.

Lemma parent_sp_same:
  forall s1 s2 cs
    (LP1: call_stack_agree init_stk cs s1)
    (LP2: call_stack_agree init_stk cs s2),
    parent_sp s1 = parent_sp s2.
Proof.
  intros.
...
Lemma lessdef_args:
forall rs rs'
  (RLD : forall r : RegEq.t, Val.lessdef (rs r) (rs' r)) l,
...
Theorem step_correct:
  forall S1 t S2 
    (STEP: Mach.step invalidate_frame1 return_address_offset ge S1 t S2)
     S1' 
    (MS: match_states S1 S1')
    (CSC1: call_stack_consistency ge init_stk S1)
    (CSC1': call_stack_consistency ge init_stk S1'),
    exists S2',
      Mach.step invalidate_frame2 return_address_offset ge S1' t S2' /\ match_states S2 S2'.
  Proof.
    destruct 1; intros S1' MS CSC1 CSC1'; inv MS; inv CSC1; inv CSC1';
    ...
    - edestruct Mem.free_parallel_extends as (m2' & FREE' & EXT); eauto.
      ...
      inv CSA. eapply Mem.free_top_tframe_no_perm; eauto.
      inv CSA0. eapply Mem.free_top_tframe_no_perm; eauto.
      eexists; split. econstructor; eauto.
      rewrite LOADV'.
      inv LD. auto. exfalso; eapply parent_ra_not_undef; eauto.
    ...
    - edestruct Mem.alloc_extends as (m1' & ALLOC & EXT1); eauto. reflexivity. reflexivity.
      ...
      + unfold in_frame; simpl. intros b [A|[]]; subst.
        repeat rewrite_stack_blocks. intro INS; apply Mem.in_stack_valid in INS.
        eapply Mem.fresh_block_alloc; eauto.
      + red; simpl. intros b fi0 o k p [A|[]]; inv A. repeat rewrite_perms.
        destr. intros.
        unfold default_frame_info. simpl. 
        destruct H3 as [H3 H3']. split.
        assumption.
        rewrite (Z.max_r _ _ (Z.le_trans _ _ _ H3 (Z.lt_le_incl _ _ H3'))).
        auto.
      + repeat rewrite_stack_blocks. auto.
      + repeat rewrite_stack_blocks.
        inv SE. omega. simpl. eapply stack_equiv_size_stack in H6. omega.
      + eexists; split. econstructor; eauto. constructor; auto.
        apply lessdef_undef_regs; auto.
        repeat rewrite_stack_blocks.
        intros A B; rewrite A, B in SE.
        ...
        intros. f_equal. rewrite ! Z.max_r in H6; auto.
        apply max_l_pos.
        apply max_l_pos.
    - edestruct (extcall_arguments_ld) as (lv & EA & LDL); eauto.
      ...
      constructor. auto.
      apply lessdef_set_pair. auto. auto.
      repeat rewrite_stack_blocks. auto.
...
Proof.
  ...
  eapply forward_simulation_step with (match_states :=
                                         fun s1 s2 =>
                                           match_states s1 s2 /\
                                           call_stack_consistency ge init_stk s1 /\
                                           call_stack_consistency ge init_stk s2).
  - reflexivity.
