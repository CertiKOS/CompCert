...
Require Import AST_old Lattice Kildall_old Memdata_old.
Require Archi.
...
Definition check_succ (s: node) (b: LTL_old.bblock) : bool :=
  match b with
  | Lbranch s' :: _ => peq s s'
  | _ => false
  end.
...
Definition pair_Iop_block (op: operation) (args: list reg) (res: reg) (s: node) (b: LTL_old.bblock) :=
...
Definition pair_instr_block
               (i: RTL_old.instruction) (b: LTL_old.bblock) : option block_shape :=
  match i with
  | Inop s =>
...
Definition pair_codes (f1: RTL_old.function) (f2: LTL_old.function) : PTree.t block_shape :=
  PTree.combine
    (fun opti optb => do i <- opti; do b <- optb; pair_instr_block i b)
    (RTL_old.fn_code f1) (LTL_old.fn_code f2).
...
Definition pair_entrypoints (f1: RTL_old.function) (f2: LTL_old.function) : option moves :=
  do b <- (LTL_old.fn_code f2)!(LTL_old.fn_entrypoint f2);
  let (mv, b1) := extract_moves nil b in
  assertion (check_succ (RTL_old.fn_entrypoint f1) b1);
  Some mv.
...
Definition transfer_aux (f: RTL_old.function) (env: regenv)
  ...
  | BStailcall sg ros args mv1 ros' =>
      ...
      assertion (opt_typ_eq sg.(sig_res) f.(RTL_old.fn_sig).(sig_res));
  ...
  | BSreturn (Some arg) mv =>
      let arg' := loc_result (RTL_old.fn_sig f) in
      do e1 <- add_equations_res arg (sig_res (RTL_old.fn_sig f)) arg' empty_eqs;
      track_moves env mv e1
  end.
...
Definition transfer (f: RTL_old.function) (env: regenv) (shapes: PTree.t block_shape)
...
Definition analyze (f: RTL_old.function) (env: regenv) (bsh: PTree.t block_shape) :=
...
Definition check_entrypoints_aux (rtl: RTL_old.function) (ltl: LTL_old.function)
                                 (env: regenv) (e1: eqs) : option unit :=
  do mv <- pair_entrypoints rtl ltl;
  do e2 <- track_moves env mv e1;
  assertion (compat_entry (RTL_old.fn_params rtl)
                          (loc_parameters (RTL_old.fn_sig rtl)) e2);
  assertion (can_undef destroyed_at_function_entry e2);
  assertion (zeq (RTL_old.fn_stacksize rtl) (LTL_old.fn_stacksize ltl));
  assertion (signature_eq (RTL_old.fn_sig rtl) (LTL_old.fn_sig ltl));
  Some tt.

Local Close Scope option_monad_scope.
Local Open Scope error_monad_scope.

Definition check_entrypoints (rtl: RTL_old.function) (ltl: LTL_old.function)
                             (env: regenv) (bsh: PTree.t block_shape)
                             (a: PMap.t LEq.t): res unit :=
  do e1 <- transfer rtl env bsh (RTL_old.fn_entrypoint rtl) a!!(RTL_old.fn_entrypoint rtl);
...
Definition check_function (rtl: RTL_old.function) (ltl: LTL_old.function) (env: regenv): res unit :=
...
Parameter regalloc: RTL_old.function -> res LTL_old.function.

(** Register allocation followed by validation. *)

Definition transf_function (f: RTL_old.function) : res LTL_old.function :=
...
Definition transf_fundef (fd: RTL_old.fundef) : res LTL_old.fundef :=
  AST_old.transf_partial_fundef transf_function fd.

Definition transf_program (p: RTL_old.program) : res LTL_old.program :=
