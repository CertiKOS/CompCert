...
Section WITHINITSP.
  ...
  Lemma parent_sp_same_tl:
    forall s1 s2 cs
           (LP1: Mach_old.list_prefix init_sg init_stk cs (tl s1)) (LP2: list_prefix init_sg init_stk cs (tl s2)) (LEN: length s1 = length s2),
  ...
  Lemma list_prefix_length:
    forall cs s,
      Mach_old.list_prefix init_sg init_stk cs s ->
...
Ltac same_hyps :=
  repeat match goal with
      ...
      Lemma step_correct:
    forall S1 t S2 (STEP: Mach_old.step init_ra return_address_offset invalidate_frame1 ge S1 t S2)
      ...
      (CSC1': call_stack_consistency ge init_sg init_stk S1'),
    exists S2',
      Mach_old.step init_ra return_address_offset invalidate_frame2 ge S1' t S2' /\ match_states S2 S2'.
  Proof.
  ...
  Lemma mach2_simulation:
    forall rao p,
      (forall (fb : block) (f : function),
          ...
          0 < fn_stacksize f) ->
      forward_simulation (Mach_old.semantics1 rao p) (Mach_old.semantics2 rao p).
  Proof.
    intros rao p SIZE.
    ...
    - simpl; intros s1 IS. eexists; split; eauto. split. eapply initial_transf; eauto.
      ...
      + simpl. constructor. repeat rewrite_stack_blocks. simpl.
        ...
        unfold Stacklayout_old.fe_ofs_arg in H2. omega.
