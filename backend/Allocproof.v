...
Lemma find_block_tailcall:
  forall tge ros ls1 ls2,
  ros_compatible_tailcall ros = true ->
  find_block tge ros (return_regs ls1 ls2) = find_block tge ros ls2.
Proof.
  unfold ros_compatible_tailcall, find_block; intros.
...
Lemma find_function_translated:
  forall ros rs fb fd ros' e e' ls,
  RTL.find_block ge ros rs = Some fb ->
  Genv.find_funct_ptr ge fb = Some fd ->
  add_equation_ros ros ros' e = Some e' ->
  satisf rs ls e' ->
  exists tfd,
  LTL.find_block tge ros' ls = Some fb /\ Genv.find_funct_ptr tge fb = Some tfd /\ transf_fundef fd = OK tfd.
Proof.
  unfold RTL.find_block, LTL.find_block; intros.
  destruct ros as [r|id]; destruct ros' as [r'|id']; simpl in *; MonadInv.
  (* two regs *)
  apply block_of_inv in H.
  exploit add_equation_lessdef; eauto. intros LD. simpl in *. rewrite H in LD. inv LD.
  simpl. rewrite pred_dec_true by reflexivity.
  exploit function_ptr_translated; eauto. intros (tf & FIND & TF). eauto.
  (* two symbols *)
...
Inductive match_states: RTL.state -> LTL.state -> Prop :=
  ...
  | match_states_call:
      forall s f args m ts tf ls m' fb
      ...
      match_states (RTL.Callstate s fb args m)
                   (LTL.Callstate ts fb ls m')
  | match_states_return:
...
Proof.
  intros. destruct H.
  constructor.
  unfold proj_sig_res in *. destruct (sig_res sg); rewrite H0; congruence.
  econstructor; eauto.
...
Lemma step_simulation:
  forall S1 t S2, RTL.step ge S1 t S2 -> wt_state prog restype S1 ->
...
- set (sg := RTL.funsig fd) in *.
  ...
  eapply plus_left. econstructor; eauto.
  eapply star_right. eexact A1. econstructor; eauto.
  rewrite <- EE. apply find_block_tailcall; auto.
...
(* internal function *)
- rewrite FIND in H; inv H. monadInv FUN. simpl in *.
  ...
  { apply wt_init_regs. inv H. rewrite wt_params. rewrite H9. auto. }
...
Let ms (w: ccworld cc) :=
  ...
  let sg := alloc_sg (snd w) in
  match_states rs (proj_sig_res sg).

Lemma initial_states_simulation:
  forall w q1 q2, match_query cc w q1 q2 ->
  forall st1, RTL.initial_state ge q1 st1 -> query_inv wt_c q1 ->
  exists st2,
    LTL.initial_state tge q2 st2 /\
    ms w st1 st2 /\
    cq_sg q1 = alloc_sg (snd w).
Proof.
  unfold ms.
  inv_compose_query.
  ...
  forall w S R q1,
    ...
    exists wA q2,
      ...
      forall r1 r2 S',
        ...
        exists R',
          LTL.after_external tge R r2 R' /\
          ms w S' R'.
Proof.
  intros w S R q HSR HS WT. subst.
  ...
  - intros r1 [ls3' m3'] S' Hr HS' Hwt.
    ...
    + econstructor; eauto.
    + econstructor; eauto.
      red; intros. rewrite (AG l H0). eapply Hls23'; eauto.
Qed.
...
Proof.
  ...
  - eapply match_reply_cc_compose.
    ...
    + apply match_reply_cc_alloc; eauto.
      ...
      destruct Archi.ptr64, (sig_res sg), (sig_res _); inv H1; reflexivity.
  - constructor.
Qed.
...
Proof.
  ...
  eapply restrict_fsim. eauto using wt_semantics, wt_prog.
  ...
  - intros w st1 t st1' Hstep1 st2 Hst.
    destruct Hstep1 as [q st1 t st1' Hstep1 Hst1 Hst1'], Hst as [Hst Hsg].
    unfold ms', ms in *. rewrite <- Hsg in Hst.
    edestruct step_simulation as (st2' & Hplus & Hst'); eauto.
    rewrite <- Hsg. eauto.
Qed.
