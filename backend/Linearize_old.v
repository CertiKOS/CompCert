...
Require Import Coqlib Maps Ordered Errors Lattice Kildall_old.
Require Import AST_old Op_old Locations_old LTL_old Linear_old.
...
Definition reachable_aux (f: LTL_old.function) : option (PMap.t bool) :=
  DS.fixpoint
    (LTL_old.fn_code f) successors_block
    (fun pc r => r)
    f.(fn_entrypoint) true.

Definition reachable (f: LTL_old.function) : PMap.t bool :=
...
Parameter enumerate_aux: LTL_old.function -> PMap.t bool -> list node.
...
Definition check_reachable_aux
     ...
     (ok: bool) (pc: node) (bb: LTL_old.bblock) : bool :=
  if reach!!pc then ok && Nodeset.mem pc s else ok.

Definition check_reachable
     (f: LTL_old.function) (reach: PMap.t bool) (s: Nodeset.t) : bool :=
  PTree.fold (check_reachable_aux reach s) f.(LTL_old.fn_code) true.

Definition enumerate (f: LTL_old.function) : res (list node) :=
...
Definition linearize_node (f: LTL_old.function) (pc: node) (k: code) : code :=
  match f.(LTL_old.fn_code)!pc with
  | None => k
  | Some b => Llabel pc :: linearize_block b k
  end.

Definition linearize_body (f: LTL_old.function) (enum: list node) : code :=
  list_fold_right (linearize_node f) enum nil.

(** * Entry points for code linearization *)

Definition transf_function (f: LTL_old.function) : res Linear_old.function :=
  do enum <- enumerate f;
  OK (mkfunction
       (LTL_old.fn_sig f)
       (LTL_old.fn_stacksize f)
       (add_branch (LTL_old.fn_entrypoint f) (linearize_body f enum))).

Definition transf_fundef (f: LTL_old.fundef) : res Linear_old.fundef :=
  AST_old.transf_partial_fundef transf_function f.

Definition transf_program (p: LTL_old.program) : res Linear_old.program :=
